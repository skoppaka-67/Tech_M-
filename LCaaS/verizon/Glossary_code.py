...
def file_iterator(path):
    ...
    :param path: Input source code path
...
def  varRelation(file_name):
    """
    Function will find the relationships between the variables based on the relationships marks the varibale is is Dead or used
    :param file_name:
    :return:
    """
    ...
    open_file=open(r'DataD_lines.txt').readlines()
    ...
    for lines in range(len(open_file)):
        ...
        try:
            ...
            print(open_file[lines][6:].strip().split())
            ...
            # print(open_file[lines][6:].strip().split()!=[])
            # print(len(open_file[lines][6:].strip().split()))
            # print(type(int(open_file[lines][6:].strip().split()[0])))
            if(open_file[lines][6:].strip().split()!=[] and len(open_file[lines][6:].strip().split())>1 and type(int(open_file[lines][6:].strip().split()[0]))==int and open_file[lines][6:].strip().split()[1].isnumeric()==False and open_file[lines][6:].strip().split()[1]!='FILLER'):
                out=[]
                act_list=[]
                while(i!=0):
                    # print(lines, i)
                    if(open_file[lines-i][6:].strip().split()!=[]):
                        if(lines-i>0):

                            # def fun88(lines):
                            #     if(open_file[lines+1][6:].strip().split()[0]=="88"):
                            #         out.append(open_file[lines+1][6:].strip().split()[1])

                            # print(open_file[lines][6:].strip().split()[0], open_file[lines-i][6:].strip().split()[0])
                            if(open_file[lines][6:].strip().split()[0]>open_file[lines-i][6:].strip().split()[0] and open_file[lines-i][6:].strip().split()[0] not in act_list  and open_file[lines-i][6:].strip().split()[0].isalnum() ):
                                # print("1",open_file[lines][6:].strip().split()[0])
                                # print("2",open_file[lines-i][6:].strip().split()[0])
                                # print("3",open_file[lines-i][6:].strip().split()[0])
                                # print("4",open_file[lines-i][6:].strip().split()[1])
                                if(open_file[lines-i][6:].strip().split()!=[] and len(open_file[lines-i][6:].strip().split())>1 and open_file[lines-i][6:].strip().split()[1]!='FILLER' and open_file[lines-i][6:].strip().split()[1]!='FILLER.'):
                                   # if()
                                # print("going inside if")
                                #     print(open_file[lines])
                                    x=open_file[lines-i][6:].split()[1].strip().replace('.','')
                                    if(x.isnumeric()!=True and open_file[lines - i][6:].strip().split()[0] != '02') :
                                        out.append(x)
                                    # print(open_file[lines])
                                    act_list.append(open_file[lines-i][6:].strip().split()[0])
                                    # print("out",out)
                            if (open_file[lines - i][6:].strip().split()[0] == '01'):
                ...
                d[open_file[lines][6:]]=out
                file_result['component_name'] = file_name.split("\\")[-1]
                if(open_file[lines][6:].split()[1]!='FILLER.'):
                    # print(open_file[lines][6:].split()[1])##= open_file[lines][6:].split()[1].replace('.','')
                    y=open_file[lines][6:].strip().split()[1].replace('.','')
...
def procedure_divison(file_entry):
   """
    ...
    Captures the procedure division lines to search for variables usage
    :param file_entry:
...
def grouping(file_entry):
    ...
    """
    Extracts the Variable declaration part to temp file
    and masks the 88 and 77 level varaibles
    ...
    with open(file_entry,'r') as f:
        ...
        for line in content:
            temp_line  = line
            # line = line[6:]
            if temp_line.replace(" ","").__contains__("LINKAGESECTION.") or temp_line.replace(" ","").__contains__("PROCEDUREDIVISION"):
                break
            ...
            if start_falg:
                ...
                # print(line[6:])
                try:
                    if line[6:].strip().split()[0] == '77':
                       # print("before ", line)
                       line=line[:6]+line[6:].replace('77', "01")
                    if line[6:].strip().split()[0] == '88':
                       # print("before ", line)
                       line = line[:6] + line[6:].replace('88', "02")
...
for file_entry in file_list:
        ...
        os.remove(r'DataD_lines.txt')## removes the temp file

        file_list_master.append(file_entry.split('\\')[-1])



################ dead var count and updating master inventory ############################
