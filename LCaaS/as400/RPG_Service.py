...
client = MongoClient('10.7.15.153', 27017)
db = client['as400']
...
def deadparalist():
    ...
    try:
        ...
        cursor = db.master_inventory_report.find(
            {"type": {"$ne": "metadata"}, "component_type": "RPG", "dead_para_count": {"$gt": 0}},
...
def getCode():
    ...
    if component_type == "BMS":
        ...
        cursy = db.codebase.find_one(
            {"$and": [{'component_name': component_name + ".DSPF"}, {'component_type': "DSPF"}]},
            {"_id": 0})
    ...
    if option is not None:
        ...
        try:
            ...
            response = db.master_inventory_report.find(
                {"application": application_name, "component_type": "RPGLE"})  # werner specific
                        ...
                        if connection['called_name'].strip() != '':
                            ...
                            if connection['component_name'].strip() == input_name:
                                if parent_child == "child":
                                    if connection['called_type'].lower() != 'file' or connection[
                                        'called_type'].lower() != 'workbook':
...
def brerule():
    ...
    for connection in connections:
            ...
            if (connection['rule_category'] == bre_connection):
                if (search_filter != ''):
                    if (len(search_filter) >= 3):
                        if (connection['pgm_name'].find(search_filter) > 0 or connection['para_name'].find(
                                search_filter) > 0 or connection['source_statements'].find(search_filter) > 0 or
                                connection['rule_category'].find(search_filter) > 0 or connection['fragment_Id'].find(
                                        search_filter) > 0 or connection['parent_rule_id'].find(search_filter) > 0 or
                                connection['business_documentation'].find(search_filter) > 0 or connection[
                                    'statement_group'].find(search_filter) > 0):
                            out_list.append(
                                {'pgm_name': connection['pgm_name'], "fragment_Id": connection['fragment_Id'],
                                 'para_name': connection['para_name'],
                                 "source_statements": connection["source_statements"],
                                 "rule_category": connection["rule_category"],
                                 "statement_group": connection["statement_group"],
                                 "parent_rule_id": connection["parent_rule_id"],
                                 "business_documentation": connection["business_documentation"]})
                else:
                    out_list.append({'pgm_name': connection['pgm_name'], "fragment_Id": connection['fragment_Id'],
                                     'para_name': connection['para_name'],
                                     "source_statements": connection["source_statements"],
                                     "rule_category": connection["rule_category"],
                                     "statement_group": connection["statement_group"],
        ...
        json_format = {
            'data': out_list,
            'headers': ["pgm_name", "para_name", "source_statements", "statement_group", "rule_category", "fragment_Id",
                        "parent_rule_id", "business_documentation"]
