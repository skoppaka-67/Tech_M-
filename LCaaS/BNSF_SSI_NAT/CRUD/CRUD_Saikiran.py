...
def ret_table(files, read_lines, table_names, retTable_next, strip_line2):
  num_flag= False
  lines = re.search("^read\s|^get\s|^find\s|^find\(|^update|^store|^delete|^select|^from", read_lines[6:].strip().lower())
  try:
    if lines.string:
      x = read_lines[6:].strip().split()
      ...
      if (len(x) >= 2 and (x[0] != "DELETE" or x[0] != "STORE" or x[0] != "UPDATE")):
        ...
        if (x[1].strip() == "NUMBER" or x[1].strip() == "FIRST" or x[1].strip() == "1" or x[1].strip().startswith("SINGLE") or x[1].strip().startswith("COUNT") or num_flag or x[1].strip() == "RECORD"):
            print("2",x )
            if (retTable_next[6:].strip().lower().startswith("from")):
                table = retTable_next.split("FROM")[1]

            elif (retTable_three[6:].strip().lower().startswith("from")):
                table = retTable_three.split("FROM")[1]
            elif read_lines[6:].strip().lower().endswith("from"):
                table = retTable_next[6:].strip()
        ...
        #         table = x[index_of_from+1]
            ...
            if read_lines[6:].strip().lower().startswith("find"):
            ...
            elif read_lines[6:].strip().lower().startswith("select") and read_lines[6:].strip().lower().__contains__("from") :
                if read_lines[6:].strip().lower().endswith("from"):
                    table= retTable_next[6:].strip()
            ...
            elif read_lines[6:].strip().lower().startswith("select") and (retTable_next[6:].strip().lower().startswith("from")
                                                                          or retTable_three[6:].strip().lower().startswith("from")):
  ...
  if (files.endswith(".nat")):
    ...
    for read_lines in line_list:
      ...
      if (table != None):
        ...
        # print("xy", xy)
        sql_query = ''
        sql_query += (open_file[xy][6:]).strip() + " "  ##adding 1st line to sql query
        next_line = 0
        ...
        while (next_line != -1):
         try:
          strip_line = open_file[xy + next_line][6:].strip().lower()
          strip_line1 = open_file[xy + next_line + 1][6:].strip().lower()
          strip_line2 = open_file[xy + next_line + 2][6:].strip().lower()
          ...
          or strip_line.startswith("delete")): #combine these lines with previous line
            ...
            if sql_query.strip() ==(open_file[xy + next_line][6:]).strip():
                # print("inside", sql_query, (open_file[xy + next_line][6:]).strip())
                sql_query+=""
            else:
                sql_query += (open_file[xy + next_line][6:]).strip() + " "
        ...
        ################ added by Kiran ################
          if strip_line.__contains__("end-select"):
             ...
             print(e, next_line, files)

        crud_val = read_lines[6:].lower().split()[0]
        ...
        if (table.isdigit()): #To go the particular line based on the number
          if (len(table) > 3):
            for new_read_line in open_file:
              if (new_read_line[:6].find(table) >= 3):
...
# r = requests.post('http://localhost:6008/api/v1/update/', json={"data": meta_data,
