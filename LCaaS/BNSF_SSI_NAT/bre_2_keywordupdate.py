...
'''Retained single quotes in source statements '''

mongoClient = MongoClient(config.database['hostname'], config.database['port'])
db = mongoClient[config.database['database_name']]
db2 = mongoClient['bnsf_nat']
# mongoClient = MongoClient('localhost', 27017)
# db = mongoClient['BNSF_NAT_POC_1']
...
def main():
 try:
    ...
    # ignore_list = ['OSPDR421.','OSPBLDTP.','OSPPOSIT.','OSPGSCHD.','OSPSVGRP.','OSPKPATH.','OSPIGOAL.','OSPCNTRN','OSPTPMSG.','OSPCGOAL.','OSPRCORE.','OSPDSTSC.','OSPSVG.']

    for component in compnentname_list:

        if component in bre_complist:
        ...
        details = collection.find({"$and": [{"pgm_name": component},{"parent_rule_id": {"$ne": ""}},
                                            # {
                                            #     '$or': [
                                            #         {"source_statements": {'$regex': ' EVALUATE '}}
                                            #         #{"parent_rule_id": {"$ne": ""}}
                                            #     ]
                                            # },
                                            {'type': {"$ne": "metadata"}}
        ...
        fetch_keyword_db()
        cprint("keywords db got hit", 'yellow')
        ...
        try:
            ...
            for i in range(len(df)):
                ...
                if j < (len(df)):
                    ...
                    if df.iloc[i, 5].__contains__("EVALUATE") and not (df.iloc[i, 5].__contains__("END-EVALUATE ")):
                        df.iloc[i, 8] = df.iloc[j, 8]
                        # print (df.iloc[i, 5], df.iloc[i, 8])
            ...
            try:
                if j < sheet.nrows:
                    ...
                    if sheet.cell_value(i, 8) != sheet.cell_value(j, 8):
                        # new_json.clear()
                        # if new_json =={}:#nested if's
                        new_json["para_name"] = new_json["para_name"]
                        # new_json["para_name"] = " <br> ".join(Remove(para_storage))
                        new_json['pgm_name'] = sheet.row_values(i)[2].replace(".", "")
                        new_json['source_statements'] = new_json['source_statements'] + " <br> " + ' '.join(storage)
                    ...
                    if flag:
                        ...
                        # para_storage.append(sheet.row_values(i)[4])
                        continue
        ...
        for index, i in enumerate(Meatadata, 1):
            ...
            # rule_description = i['source_statements']
            rule_description = keyword_rep(i['source_statements'])

            Rule = i['parent_rule_id']
            rule_relation = ""
            # print(bre2_json)
            bre2_json = {"fragment_id": component + "-" + str(index), "pgm_name": pgmname.replace(".", ""),
                         "para_name": paraname, "source_statements": source_statements, 'rule_category': " ",
                         'Rule': Rule,
                         'rule_relation': rule_relation, "rule_description": rule_description}
        ...
        try:
            ...
            if db.bre_report2.update_one({"type": "metadata"}, {"$set": {"last_updated_on": current_time,
                ...
                cprint(
                    "Data for " + component + " Genrated..... and Inserted to DataBase ..! at " + str(process_end_time),
                    'magenta')
        except Exception as e:
            from datetime import datetime
            exc_type, exc_obj, exc_tb = sys.exc_info()
