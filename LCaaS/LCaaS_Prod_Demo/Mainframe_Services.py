...
# # client = MongoClient('172.31.23.144', 27017)
...
def getCode():
    ...
    if not re.match('\S+\.\S+', component_name.strip()):
        ...
        master_component_types = {'COBOL': 'cbl', 'COPYBOOK':'cpy','JCL':'jcl','PROC':'proc','SYSIN':'sysin','SCR':'scr','SCRIPTS':'scr'} #This line to be copied from the script
...
#                 # cursor = db.varimpactcodebase.find({'$text':{'$search':search_filter }}, {'_id': 0})
#                 # Query to fetch header details
#                 for i in cursor:
#                     temp_json = i
#                     temp_json["child_components"] = []
#                     temp_json["parent_components"] = []
#                     app = db.master_inventory_report.find_one({"component_name": temp_json["component_name"]})
#                     x_ref_app_ch = db.cross_reference_report.find(
#                         {"component_name": temp_json["component_name"].split(".")[0]})
#                     x_ref_app_p = db.cross_reference_report.find(
#                         {"called_name": temp_json["component_name"].split(".")[0]})
#                     #
#                     for c in x_ref_app_ch:
#                         temp_json["child_components"].append(c["called_name"] + "[" + c["called_type"] + "]")
#                     #
#                     for p in x_ref_app_p:
#                         temp_json["parent_components"].append(p["component_name"] + "[" + p["component_type"] + "]")
#
#                     temp_json["parent_components"] = ",".join(temp_json["parent_components"])
#                     temp_json["child_components"] = ",".join(temp_json["child_components"])
#
#                     temp_json['application'] = app['application']
#                     data_object['data'].append(copy.deepcopy(temp_json))
#                 metadata = db.varimpactcodebase.find_one({"type": "metadata"}, {'_id': 0})
#                 data_object['headers'] = metadata['headers']
#
#                 # Pandas code for cursor to dictionary convertion
...
#                     x_ref_app_ch = db.cross_reference_report.find(
#                         {"component_name": temp_json["component_name"].split(".")[0]})
#                     x_ref_app_p = db.cross_reference_report.find({"called_name": temp_json["component_name"].split(".")[0]})
#                     #
#                     for c in x_ref_app_ch:
#                         temp_json["child_components"].append(c["called_name"] + "[" + c["called_type"] + "]")
#                     #
...
#                 data_object['headers'] = metadata['headers']
#                 data_object['displayed_count'] = len(data_object['data'])
#                 print("endtime", timeit.default_timer())
#                 return jsonify(data_object)
#         else:
#
#             if search_filter:
...
#                             temp_json["parent_components"].append(p["component_name"] + "[" + p["component_type"] + "]")
#
#                         temp_json["parent_components"] = ",\n".join(temp_json["parent_components"])
#                         temp_json["child_components"] = ",\n".join(temp_json["child_components"])
#
#                         temp_json['application'] = app['application']
#                         data_object['data'].append(copy.deepcopy(temp_json))
#                     metadata = db.varimpactcodebase.find_one({"type": "metadata"}, {'_id': 0})
...
#                         temp_json["parent_components"] = []
#                         app = db.master_inventory_report.find_one({"component_name" :temp_json["component_name"]})
#                         x_ref_app_ch = db.cross_reference_report.find({"component_name":temp_json["component_name"].split(".")[0]})
#                         x_ref_app_p = db.cross_reference_report.find({"called_name":temp_json["component_name"].split(".")[0]})
#
#                         for c in x_ref_app_ch:
#                             temp_json["child_components"].append(c["called_name"]+"["+c["called_type"]+"]")
...
def search(keyword):
        ...
        body = {
            "query": {
                "multi_match": {
                    "query": keyword,
                    "fields": ["sourcestatements"]
                }
            }
        }

        res = es.search(index="screenfields", doc_type="title", body=body)
        return res
        ...
        if override_filter == 'yes':
            ...
            if search_filter:
                ...
                if exact_match_filter == "yes" :
                    ...
                    data_object['data']= search(search_filter)
                    metadata = db.varimpactcodebase.find_one({"type": "metadata"}, {'_id': 0})
                    data_object['headers'] = metadata['headers']
                    """
...
def comment_lines():
    ...
    for cursor1 in cursor:
        json_format = {"component_name": cursor1["component_name"], "component_type": cursor1["component_type"],
                       "commented_lines": cursor1["comment_line"]}
    ...
    if option is not None:
        ...
        try:
            ...
            if response is None:
                ...
                for appln in response:
                    comp_name_set.add(appln["component_name"])
                print({"component_list": sorted(list(comp_name_set)), "application_name": application_name})
                return jsonify({"component_list": sorted(list(comp_name_set)), "application_name": application_name})
        except Exception as e:
            print("Error", str(e))
            return jsonify({"status": "failure", "reason": "Error: " + str(e)})
