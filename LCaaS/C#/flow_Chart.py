...
class PreProcessing():
    ...
    def get_files(self):
        filenames_list = []
        for filename1 in glob.glob(os.path.join(vb_path, '*.aspx.cs')):
            filenames_list.append(filename1)
        # for filename2 in glob.glob(os.path.join(vb_component_path, '*.cs')):
        ...
        for line in f.readlines():
            if line.strip().startswith("//"):
                ...
                if line.strip().startswith("If") and not (line.strip().endswith("Then")):
                    target_file.write(line.replace("=>","lamda_op"))
    ...
    def tagging_end_if_to_singleline_id_for_each_function(self, lines_list):
        ...
        for index1,line in enumerate(line_list):
            ...
            if line.strip().startswith("catch")or line.strip().startswith("try") or line.strip().startswith("foreach") or line.strip().startswith("else")or line.strip().startswith("for") or line.strip().startswith("case") or  line.strip().startswith("default") or  line.strip().startswith("switch") :
...
class flowchartProcessing(PreProcessing, Database):
    ...
    def __init__(self, DB_CON, pre_prop):
        ...
        cursy = DB_CON.Db_conn("C#", "Pub_Fun_Lookup")
    ...
    def block_separation(self, code_block_dict,file_name):
        ...
        # print(json.dumps(OUTPUT_DATA,indent=4))
            ...
            # Insert into DB
        try:
            DB_CON.db_update("C#","para_flowchart_data",OUTPUT_DATA)
            OUTPUT_DATA.clear()
        ...
        for index, iter in enumerate(index_list):
            ...
            if IF_VAR in index_list[index] and END_IF_VAR in index_list[alter_index]:
                ...
                if len(index_list) > alter_index + 1:
                    if ELSE_VAR in index_list[alter_index+1] and  list(index_list[alter_index].values())[0] + 1 == list(index_list[alter_index+1].values())[0]:
                        continue
                    else:
            ...
            if (ELSE_VAR in index_list[index] and IF_VAR in index_list[alter_index]):
                    ...
                    false_block_var = line_list[
                                      list(index_list[index].values())[0] + 1: list(index_list[alter_index].values())[
                                          0]]
                    if END_IF_VAR in index_list[index- 1] and list(index_list[index-1].values())[0] +1 == list(index_list[index].values())[0]:
                        false_block_count = pop_list[-1]
