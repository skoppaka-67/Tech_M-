...
ConditionPath=config.COBOL_codebase_information['condition_path']
...
def main():
 ...
 wb1 = xlrd.open_workbook("D:\\Book2.xlsx")
 ...
 for filename in glob.glob(os.path.join(CobolPath,'*.cbl')):
         ...
         if line[0]=='*'or line.strip()=="":
          ...
          if perform!=[]:
              ...
              if perform[:-1] in paraRepository.keys():
                  ...
                  temp_file1.write("           END PARA -("+ perform[:-1]+")"+"   ^"+ currentmodulevalue)
    ...
    with open("FinalFile6" + str(i) + '.txt') as infile, open('output'+str(i)+'.txt', 'w') as outfile:
        for line in infile:
                ...
                if previousDeleted  or UpdateCounter ==1:
                    ...
                    try:
                        ...
                        if db.bre_rules_report.count_documents({"type": "metadata"}) > 0:
                            print(db.bre_rules_report.update_one({"type": "metadata"},
                                                                  {"$set": {
