...
for ite in res:
    if ite['_id']!= {}:
        ...
        if component_name1.__contains__('.dclgen')  :
           master_inv_set.add(component_name1[:-7]+' '+ite['_id']['component_type'])
        else:
...
ignored_types_list=['File']
for ite in cross_reference:
    if ite['_id'] != {}:
        #print(ite['_id'])
        if ite['called_type'] not in ignored_types_list:
            cross_ref_set.add(ite['called_name']+' '+ite['called_type'])
