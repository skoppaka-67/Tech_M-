...
def expandCopyooks(file_pointer, copybook_location):
    ...
    for line in file_pointer:
        ...
        if line.strip() != '' :
            if line[0] != '*':
                # print("Copy:",line)
                if re.match('\s*copy *', line, re.IGNORECASE) or re.match('\s*\+\+INCLUDE\s+.*', line, re.IGNORECASE):
...
for COMPONENT_NAME in COBOL_FILE_LIST:
    ...
    '''
    keywords_for_if_delimiter = ['ACCEPT', 'RETURN', 'STOP RUN', 'END-PERFORM', 'CALL', 'MOVE', 'COMPUTE', 'PERFORM',
