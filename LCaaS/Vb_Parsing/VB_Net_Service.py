...
def procedureFlowCompList():
    ...
    if option is not None:
        ...
        try:
            response = db.master_inventory_report.find({"application": application_name, "component_type":{"$in":["WEBFORM","USERCONTROL","GLOBAL.ASAX","MASTERPAGE"]}})
            if response is None:
...
def expandedgetCode():
    ...
    start = timeit.default_timer()
    cursy = db.expanded_codebase.find_one({"$and": [{'component_name': component_name}, {'component_type': component_type.upper()}]},
                                 {"_id": 0})
    end = timeit.default_timer()
    if component_line != None:
        replaced_lines = ""
