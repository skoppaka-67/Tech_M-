...
class pub_sh_lookup:
    ...
    def build_lookup(self,file_list):
            ...
            self.db_delete("Vb_net1", "Pub_Fun_Lookup")
            self.db_update("Vb_net1", "Pub_Fun_Lookup", DB_Data)
    ...
    def get_files():
        ...
        for filename2 in glob.glob(os.path.join(vb_path, '*.aspx')):
            filenames_list.append(filename2)

        for filename2 in glob.glob(os.path.join("D:\\VB_IMP\\ascx", '*.ascx')):
            filenames_list.append(filename2)
        for filename2 in glob.glob(os.path.join("D:\\VB_IMP\\Main", '*.asax')):
            filenames_list.append(filename2)
...
class Xref(pub_sh_lookup):
    ...
    def fetch_file_list(self, filelist):
        ...
        for file in filelist:
            dicts["file_name"] = file.split("\\")[-1].split(".")[0]
            dicts['lob_name'] = file.split(("\\"))[-2]
    ...
    def scan_for_stored_proc(self,code_block_dict,file):
        ...
        for fun , line_list in code_block_dict.items():
            ...
            if command_type:
                op_json["component_name"] = file.split("\\")[-1]
                op_json['component_type'] = file.split("\\")[-1].split(".")[1].replace("aspx", "CODEBEHIND")
                op_json['calling_app_name'] = file.split("\\")[-2].replace(".BusinessRules", '').upper()
                op_json['called_name'] = store_proc_name
                op_json['called_type'] = 'Store Proc'
                op_json['called_app_name'] =  file.split("\\")[-2].replace(".BusinessRules", '').upper()
    ...
    def process(self):
        ...
        cursy = self.Db_conn("Vb_net1", "Pub_Fun_Lookup")
        # print(ref_file_list)
        for file in file_list:

            if file.endswith(".aspx")  or file.endswith(".ascx") or file.endswith(".asax") :
                f = open(file, encoding="utf8")
                for line in f.readlines():
                    if line.__contains__("MasterPageFile="):
                        called_master_name = line.split("MasterPageFile=")[1].split()[0].replace("~/","")
                        op_json["component_name"] = file.split("\\")[-1].split(".")[0]
                        op_json['component_type'] = "WEBFORM"
                        op_json['calling_app_name'] = file.split("\\")[-2].replace(".BusinessRules", '').upper()
                        op_json['called_name'] = called_master_name.replace('"',"").replace("'","")
                        op_json['called_type'] = 'MASTERPAGE'
                        op_json['called_app_name'] = file.split("\\")[-2].replace(".BusinessRules", '').upper()
                        META_DATA.append(copy.deepcopy(op_json))

                    if line.__contains__("CodeBehind="):
                        called_name = line.split("CodeBehind=")[1].split()[0].replace(".BusinessRules", '')

                        op_json["component_name"] = file.split("\\")[-1].split(".")[0]
                        op_json['component_type'] = "WEBFORM"
                        op_json['calling_app_name'] = file.split("\\")[-2].replace(".BusinessRules", '').upper()
                        op_json['called_name'] = called_name.replace('"',"").replace("'","").split(".")[0]
                        op_json['called_type'] = 'CODEBEHIND'
                        op_json['called_app_name'] = file.split("\\")[-2].replace(".BusinessRules", '').upper()
                        META_DATA.append(copy.deepcopy(op_json))
                    if line.__contains__("href="):

                        called_name = self.get_href_value(line)

                        op_json["component_name"] = file.split("\\")[-1].split(".")[0]
                        op_json['component_type'] = "WEBFORM"
                        op_json['calling_app_name'] = file.split("\\")[-2].replace(".BusinessRules", '').upper()
                        op_json['called_name'] = called_name.replace('"', "").replace("'", "")
                        op_json['called_type'] = 'HYPERLINK'
                        op_json['called_app_name'] = file.split("\\")[-2].replace(".BusinessRules", '').upper()
                        META_DATA.append(copy.deepcopy(op_json))
            ...
            for line in f.readlines():
                ...
                if matches1 != []:
                    ...
                    op_json['called_app_name'] = val['called_app_name'].upper()
                    META_DATA.append(copy.deepcopy(op_json))

                matches2 = [x for x in ref_file_list if " "+x+" " in line and not (line.__contains__("Public"))and not (line.__contains__("Private"))and not (line.__contains__("Protected"))and not (line.__contains__("Dim"))]
                if matches2 != []:
                    # print(matches2,line)
                    ...
                    op_json["component_name"] = file.split("\\")[-1].split(".")[0]
                    op_json['component_type'] = file.split("\\")[-1].split(".")[1].replace("aspx", "CODEBEHIND").upper()
                    op_json['calling_app_name'] = file.split("\\")[-2].replace(".BusinessRules", '').upper()
                    op_json['called_name'] = matches2[0]
                    op_json['called_type'] = 'VB'
                    op_json['called_app_name'] = val['lob_name'].upper()
                    META_DATA.append(copy.deepcopy(op_json))
                matches22 = [x for x in ref_file_list if
                            " " + x +"(" in line and not (line.__contains__("Public")) and not (
                                line.__contains__("Private")) and not (line.__contains__("Protected"))]
                if matches22 != []:
                    # print(matches2,line)
                    # self.json_maker(file, ref_file_dict_list, matches2, "file_name", True)
                    val = next(item for item in ref_file_dict_list if item["file_name"] == matches22[0])
                    op_json["component_name"] = file.split("\\")[-1].split(".")[0]
                    op_json['component_type'] = file.split("\\")[-1].split(".")[1].replace("aspx", "CODEBEHIND").upper()
                    op_json['calling_app_name'] = file.split("\\")[-2].replace(".BusinessRules", '').upper()
                    op_json['called_name'] = matches22[0]
                    op_json['called_type'] = 'VB'
                    op_json['called_app_name'] = val['lob_name'].upper()
                    META_DATA.append(copy.deepcopy(op_json))
                ...
                if matches3 !=[]:
                    ...
                    op_json['component_type'] = file.split("\\")[-1].split(".")[1].replace("aspx", "CODEBEHIND").split(".")[0]
                    op_json['calling_app_name'] = file.split("\\")[-2].replace(".BusinessRules", '').upper()
                    op_json['called_name'] = val["File_name"]
                    op_json['called_type'] = val["File_name"].split(".")[1].replace("aspx", "CODEBEHIND").upper()
                    op_json['called_app_name'] = val['Lob_name'].upper()
                    META_DATA.append(copy.deepcopy(op_json))
                ...
                if line.__contains__("Response.Redirect"):
                    ...
                    if line.__contains__("~/"):
                        op_json['called_name'] = line.split("/")[-1].replace(".aspx","").replace(")","").replace('"',"").strip().split(".")[0]

                    else:
                        op_json['called_name'] = line.split("(")[1].split(")")[0].replace('"',"").replace("'","").strip().replace(".aspx","").strip().split(".")[0]
                    op_json['called_type'] = "WEBFORM"
                    op_json['called_app_name'] =file.split("\\")[-2].upper()
                    META_DATA.append(copy.deepcopy(op_json))
            store_proc_jsons = self.scan_for_stored_proc(code_blocks_dict,file)
            f.close()

    def get_href_value(self,line):

        called_name = line.split("href=")[1].split(">")[0].split()[0]
        print(called_name.replace('"', "").replace("'", ""))
...
if __name__ == '__main__':
    ...
    META_DATA.append({ "type" : "metadata","headers" : [
            "component_name",
            "component_type",
            "called_name",
            "called_type",
    ...
    pubShLokUp.db_delete("vb_update","cross_reference_report")
    pubShLokUp.db_update("vb_update","cross_reference_report",xref.remove_duplicates(META_DATA))
