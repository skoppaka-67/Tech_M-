...
def procedureFlowCompList():
    ...
    if option is not None:
        ...
        try:
            response = db.master_inventory_report.find({"application": application_name, "component_type":{"$in":["WEBFORM","USERCONTROL","GLOBAL.ASAX","MASTERPAGE","CODEBEHIND","VB"]}})
            if response is None:
...
def procFlowChartPOST():
    ...
    try:
        if para_name.__contains__("("):
            flowchart_metadata = db.para_flowchart_data.find_one(
                {"$and": [{"component_name": para_name.split("(")[1][:-1]}, {"para_name": para_name.split("(")[0]}]},{'_id': 0})
        ...
        if flowchart_metadata is None:
            ...
            flowchart_metadata = db.para_flowchart_data.find_one(
                {"$and": [{"component_name": component_name+'.vb'}, {"para_name": para_name.strip()}]}, {'_id': 0})
...
def getCode():
    ...
    if not re.match('\S+\.\S+', component_name.strip()):
        ...
        master_component_types = {'CODEBEHIND': 'aspx.vb', 'JAVASCRIPT':'js','WEBFORM':'aspx','VB':'vb'} #This line to be copied from the script
...
def expandedgetCode():
    ...
    if para_name.__contains__("("):
        ...
        component_name = component_name+".vb"
    ...
    if not re.match('\S+\.\S+', component_name.strip()):
        # Component does not have extension,
        master_component_types = {'WEBFORM': 'aspx', 'JAVASCRIPT': 'js', 'CODEBEHIND': 'aspx.vb', 'VB': 'vb'
                                  }  # This line to be copied from the script
        #Search for component extension via lookup and
        if component_type in master_component_types:
            component_name = component_name.strip() + '.' + master_component_types[component_type]
...
def cycComplex():
    ...
    application = []
    orphan_data1 = []
    cursor = db.master_inventory_report.find({"$or":[{"component_type": "CODEBEHIND"},{"component_type": "VB"}]})
                    ...
                    for item in node_set:
                        ...
                        for linkdict in pgm_name_1:
                            if linkdict["to"] == item[2:] and linkdict["name"]=="External":
                                ...
                                #print(tempdict["label"], linkdict["file"], linkdict["to"] ,item[2:])
                            elif (linkdict["name"]=="internal" or linkdict["name"]=="Webform") and linkdict["to"] == item[2:] :
                                tempdict["label"] = item[2:]
                        print(tempdict)
                        if len(tempdict.keys())==1:
                            tempdict["label"]=tempdict["id"][2:]
...
def controlflow():
    ...
    try:
            ...
            if response['component_type'] == 'VB':
             table_connections = db_link2(response['component_name']+'.vb',response['component_type'])

            elif response['component_type'] == 'CODEBEHIND':
                com_name=response['component_name'] + '.aspx.vb'
                table_connections = db_link2(com_name, response['component_type'])

            for i in table_connections:

                if i["component_name"].split('.')[0] == response['component_name']:  # inputcomponent:
                    print("insed")
                    response["nodes"].append({"id": "p_" + i["Table"],"label": i["Table"] + "(" + "Table" + ")"})
                    response["links"].append({"label": i["CRUD"],
                                              "source": "p_" + i["component_name"].split('.')[0]+"(" + i["component_type"] + ")",
                                              "target": "p_" + i["Table"]})
        ...
        if option is not None:
            if flag == "no":
                ...
                try:
                    ...
                    response =  db.procedure_flow_table.find_one({"$and":[{"component_name":program_name, "event_name" :event }]}, {'_id': 0})
                    print(response)
                    if response is None:
                        return jsonify({"response":"No such key"})
                    else:
                        ...
                        if connection['called_name'].strip() != '':
                            ...
                            if connection['component_name'].strip() == input_name:
                                if parent_child == "child":
                                    if connection['called_type'].lower() != 'file' and connection['called_type'].lower() != 'workbook' and connection['called_name'] not in component_name:
...
def comment_lines():
    ...
    #print(name)
    cursor = db.cobol_output.find({"component_name": name})
    json_format = ""
...
def procFlowChart_RD():
    ...
    try:
        ...
        if para_name.__contains__("("):
            ...
            print(component_name.replace(".vb","")+".vb")
            flowchart_metadata = db.translated_flowchart.find_one(
                {"$and": [{"component_name": component_name.replace(".vb","")+".vb"}, {"para_name": para_name}]},
                {'_id': 0})
...
#     if apname == "All":
#         try:
#             cursor = db.screen_feilds.find({'type': {"$ne": "metadata"}},
#                                           {"filename": 1, "Application": 1, "SectionID": 1,
#                                            "SectionName": 1, "ScreenField": 1, "Type": 1,
#                                            "TextMode": 1, "Enable": 1, "Tooltip": 1, "Visible": 1,
#                                            "DataTextField": 1, "DataValueField": 1, '_id': 0})
...
#     else:
#         try:
#             cursor = db.screen_feilds.find( { "$and": [ {"Application": apname}] },{"filename": 1, "Application": 1, "SectionID": 1,
#                                            "SectionName": 1, "ScreenField": 1, "Type": 1,
#                                            "TextMode": 1, "Enable": 1, "Tooltip": 1, "Visible": 1,
#                                            "DataTextField": 1, "DataValueField": 1, '_id': 0})
#             # cursor = db.screen_feilds.find( { "$and": [ {"Application": apname}, { "filename":filename } ] },{"filename": 1, "Application": 1, "SectionID": 1,
#             #                                "SectionName": 1, "ScreenField": 1, "Type": 1,
#             #                                "TextMode": 1, "Enable": 1, "Tooltip": 1, "Visible": 1,
#             #                                "DataTextField": 1, "DataValueField": 1, '_id': 0})
#
...
#     else:
#         try:
#             cursor = db.screen_feilds.find({"Application": apname, "filename": filename},
#                                           {"filename": 1, "Application": 1, "SectionID": 1,
#                                            "SectionName": 1, "ScreenField": 1, "Type": 1,
#                                            "TextMode": 1, "Enable": 1, "Tooltip": 1, "Visible": 1,
#                                            "DataTextField": 1, "DataValueField": 1, '_id': 0})
...
# @app.route('/showvalidators', methods=['GET'])
# def showvalidators():
#     option = request.args.get('option')
#     option1 = request.args.get('option1')
#     cursor = list(col.find({"filename":option1,"Application":option,},{"_id":0}))
    ...
    try:
        ...
        if override_filter == 'yes':
            ...
            if search_filter:
                ...
                cursor = db.bre_rules_report.find({'$and':[{'type': {"$ne": "metadata"}},{"rule_category" : option}]}, {'_id': 0})
                # Query to fetch header details
                metadata = db.bre_rules_report.find_one({"type": "metadata"}, {'_id': 0})
                data_object['headers'] = metadata['headers'][1:]
    ...
    if apname == "All":
        ...
        for cursor in cursors:
            json_format = {"component_name": cursor["component_name"],
                           "application": cursor["application"].upper()}
            if (json_format not in output_list):
                output_list.append(json_format)
        output_json = {"data": output_list, "headers": ["component_name",  "application"]}
        print(output_json)
...
def bre_2():
    ...
    try:
        if option == "":
            ...
            cursor = db.rule_report.find({"pgm_name": option.strip()}, {"_id": 0}).sort('_id')
            datavalue["headers"] = header
            for record in cursor:
...
def chartsAPI():
    ...
    try:
        ...
        try:
            ...
            OUTPUT_DATA['orphan_stats']['orphan_ components'] = db.orphan_report.count_documents({})
...
def nameList():
    ...
    print("----->>   Selected--app",selectedApplication)
    # selectedApplication = "ANONYMOUS"
    if selectedApplication == "All":
        return jsonify({'name_list': []})
    else:
        cursor = db.screen_feilds.find({"Application": selectedApplication}, {"filename": 1, '_id': 0})
        output = []
        for document in cursor:
            if "filename" in document.keys():
...
def validationreport():
    ...
    option = request.args.get('option')
    option1 = request.args.get('selectedName')
    # cursor = col.find_one({},{"_id":0})
    cursor = list(col.find({"filename": option1, "Application": option},
                           {"filename": 1, "Application": 1,
                            "ScreenField": 1, "Type": 1,
                            "propertyvalidation": 1, "validators": 1,
                            "CodeValidation": 1, "_id": 0}))
    data_object['data'] =  cursor
    data_object['headers'] = ["filename","Application","ScreenField","Type","propertyvalidation","validators","CodeValidation" ]
...
@app.route('/api/v1/BreCompList', methods=['GET'])
def BreCompList():
    option = request.args.get('option')
    comp_name_set = set()
    if option is not None:
