...
def main(filename, metadata):
    try:
                ...
                if data["field_name"] == control_id.strip():
                    ...
                    for condition_data_line_1 in condition_split_list:
                        ...
                        if condition_data_line.upper().strip().startswith(
                            ...
                            data1 = {"function_name": data["function_name"], "field_name": data["field_name"],
                                     "rule_id": "rule-" + str(rule), "rule_statement": condition_data_line_1,
                                     "Rule_Relation": "", "rule_description": "",
                            ...
                            db.rule_report.insert_one(
                                {"function_name": data["function_name"], "field_name": data["field_name"],
                                 "rule_id": "", "rule_statement": condition_data_line_1, "Rule_Relation": "",
                                 "rule_description": "",
                                 "rule_type": "", "file_name": filename.split('\\')[-1], "lob": "",
                                 "dependent_control": "", "parent_rule_id": ",".join(parent_rule_id)})
                            for j in condition_data_line_1:
        ...
        with open("screen_fields1" + '.csv', 'w', newline="") as output_file:
            Fields = ["rule_id", "file_name", "function_name", "field_name", "rule_statement", "Rule_Relation",
                      "rule_description", "rule_type", "lob", "dependent_control", "parent_rule_id"]
...
def spliting_of_condition_statement(data):
    try:
        ...
        for condition_line in data['rule_statement']:
            ...
            if if_regexx_1 != [] or if1_regexx_1 != [] or select_flag_regexx_1 != [] or case_regexx_1 != []:
...
def classification_of_if(data):
    try:
        ...
        for line_2 in data["rule_statement"]:
            if (line_2.strip().startswith("If ") or line_2.strip().startswith("If(")) and line_2.strip().endswith(
                    ' Then'):
    ...
    try:
        ...
        if not inside_flag:
            for data_1 in if_cursor["source_statement"]:
                if data_1.split("=")[0].strip().__contains__(name_val_var.strip()) and data_1.__contains__(
