...
Version9 = "Fully working code after fixing New patterns"

output_json = {"filename": "", "lob": "", "section_id": "", "section_name": "", "CLAS_label": "", "comments": "",
            ...
            for line in input_file.readlines():
                ...
                if section_flag or nested_counter >= 1:
                    ...
                    expand_item_regexx = re.findall(
                        r'^\s*<\s*.*\s*:\s*ExpandCollapsePanelItem\s*.*id\s*=\s*"\s*[aA-zZ]*"', line, re.IGNORECASE)
                    stack_item_regxx = re.findall(r'^\s*<\s*.*\s*:\s*StackingPanelItem\s*.*id\s*=\s*"\s*[aA-zZ]*"',
                                                  line, re.IGNORECASE)
                    ...
                    if control_regexx != []:
                        ...
                        if control_name.upper() == "LABEL":
                            ...
                            json_value = {"filename": filename.split('\\')[len(filename.split('\\')) - 1],
                                          "lob": filename.split('\\')[len(filename.split('\\')) - 2], "SectionID": "",
                ...
                if content.lower().__contains__("datasource") and content.lower().__contains__(".lookupdata"):
                    ...
                    with open(filename + '.vb', 'r') as vb_file:
                        ...
                        for lines in file_handle:
                            if lines.__contains__(lookup_ref) and lines.__contains__("=") and not lines.lower().__contains__(".datasource"):
                                ...
                                if lookup_fun_store != []:
                                    for l in lookup_fun_store:
                                        if l.__contains__("LookUpDropDownValue") and l.lower().__contains__("new") or brace_counter >=1:
                                            for i in l.strip():
                                                if i == '(':
                                        ...
                                        if singleline_formation_flag:
                                            ...
                                            if not split_list[0].lower().__contains__('nothing'):
                                                ...
                                                key_list.append(split_list[0].split("(")[1].split(")")[0].replace('"',""))
                                                ...
                                                if result_line != '':
                                                    if result_line.__contains__("="):
                                                        ...
                                                        VB_PATH = find_VBpath(VBpath_class_var, VBpath_funtion_name,filename)
                                                        dropdown_vaues_list1.append(fetch_stored_values_drop_down1(VB_PATH, VBpath_funtion_name,key_list))
                                                ...
                                                dropdown_vaues_list.append("DataTableSource: " +final_dropdown_vaues_list[0].split(",")[0]+",Text:"+split_list[1]+",Value:"+split_list[2]+",ControlAliasName:"+split_list[3]+",iLookupApplicable:"+split_list[4]+",Policyeffectivedate:"+split_list[5]+",Lob:"+split_list[6]+",State:"+split_list[7]+",isDepEligible:"+split_list[8]+",DepAliasName:"+split_list[9]+",DepValue:"+split_list[10]+",DependentKey:"+split_list[11])
                                                singleline_formation_flag = False
                                                singleline = ""


                                            else:
                                                dropdown_vaues_list.append("DataTableSource: " +split_list[0]+",Text:"+split_list[1]+",Value:"+split_list[2]+",ControlAliasName:"+split_list[3]+",iLookupApplicable:"+split_list[4]+",Policyeffectivedate:"+split_list[5]+",Lob:"+split_list[6]+",State:"+split_list[7]+",isDepEligible:"+split_list[8]+",DepAliasName:"+split_list[9]+",DepValue:"+split_list[10]+",DependentKey:"+split_list[11])
...
def tooltipdervived_var1(path, with_name_by_Parm_pos, function_name, keyvalue):
    ...
    for line in storage:
        if line.__contains__(with_name_by_Parm_pos + ".") and line.__contains__(keyvalue):
            line = line.split('(')[1].split(')')[0]  # IDIOM to Fetch DATA BETWEEN Parameters
            if line.split(",")[1].startswith('"'):
                return line.split(",")[1]
