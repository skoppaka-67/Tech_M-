...
def main(filename, metadata):
    try:
                ...
                if data["field_name"] == control_id.strip():
                    ...
                    data.update(
                        {"rule_id": "rule-" + str(rule), "parent_rule_id": "", "rule_description": "",
                         "rule_type": "server_side", "file_name": filename.split('\\')[-1], "lob": "",
                         "dependent_control": ""})
        ...
        with open("Rules_Report_2" + '.csv', 'w', newline="") as output_file:
            Fields = ["rule_id", "file_name", "function_name", "field_name", "rule_statement", "parent_rule_id",
                      "rule_description", "rule_type", "lob", "dependent_control"]
    ...
    with open(filename + '.vb', 'r') as input_file:
        ...
        for line in input_file.readlines():
            ...
            if end_sub != [] or end_function != []:
                metadata_1.append({"file_name": filename.split('\\')[-1].split('.')[0], "function_name": function_name,
                                   "source_statement": full_data_list})
                dup_flag = False
                full_data_list = []
            ...
            # if end_if_regexx != [] or if_counterer >= 2:
            ...
            if end_if_regexx != []:
                ...
                if if_counterer == 0:
                    ...
                    metadata.append(
                        {"function_name": function_name, "field_name": cid_name, "rule_statement": if_data_list,
...
def classification_of_if(data):
    ...
    for line_2 in data["rule_statement"]:
        if (line_2.strip().startswith("If ") or line_2.strip().startswith("If(")) and line_2.strip().endswith(' Then'):
            ...
            break
        elif (line_2.strip().startswith("If ") or line_2.strip().startswith("If(")) and not line_2.strip().endswith(
                ' Then'):
    ...
    if if_statement_string.__contains__('(') and if_statement_string.__contains__(')'):  # checking for function.
        ...
        with open(filename + '.vb', 'r') as vb_file:
            for lines in vb_file.readlines():
                ...
                if (lines.strip().startswith("If ") or lines.strip().startswith("If(")) and lines.strip().endswith(
                        ' Then'):
                    lines = lines.strip()

                elif (lines.strip().startswith("If ") or lines.strip().startswith(
                        "If(")) and not lines.strip().endswith(' Then'):
    ...
    try:
        ...
        for lines in full_function_data:
            ...
            for in_line in lines["source_statement"]:
                ...
                #     back_track_if_counterer=back_track_if_counterer+1
                #
                    ...
                    for rec_data in rules_lines:
                        for k in rec_data["rule_statement"]:
                            ...
                            if function_name_regexx_1 != []:
                                db.rule_report.insert_one({"dependent_control": dup_data["field_name"],
                                                           "function_name": rec_data["function_name"],
...
#
#                 rule_statements[function_name + " "] = if_to_end_if_collecter(cursor['source_statement'],
#                                                                               line)  # to allow duplicates in Dictionary
#             else:
...
#     with open(code_location + "\\WebApplications\\" + filename + '.vb', 'r') as vb_file:
#         for lines in vb_file.readlines():
#             if lines.upper().__contains__("AS") and lines.__contains__(".Lob") or lines.__contains__(" Lob") and (
