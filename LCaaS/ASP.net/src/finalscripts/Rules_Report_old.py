...
def main(filename,metadata):
      ...
      try:
                ...
                if data["field_name"] == control_id.strip():
                    ...
                    for condition_data_line_1 in condition_split_list:
                        ...
                        if condition_data_line.upper().strip().startswith("IF") or condition_data_line.upper().strip().startswith("SELECT ") or \
                            ...
                            data1={"function_name":data["function_name"],"field_name":data["field_name"],"rule_id": "rule-" + str(rule),"rule_statement":condition_data_line_1, "Rule_Relation": ",".join(parent_rule_id), "rule_description": "",
        ...
        with open("location_rule" + '.csv', 'w', newline="") as output_file:
            Fields = ["rule_id","file_name","function_name","field_name","rule_statement","Rule_Relation","rule_description","rule_type","lob"]
    ...
    try:
        ...
        for line in dup_data['rule_statement']:
            ...
            if matches1 != []:
                ...
                calling_function_name = line.split(matches1[0])[1].split('(')[0].replace(".", "")
                cursor1 = db.master_rules.find({"function_name": calling_function_name}, {'_id': 0})
