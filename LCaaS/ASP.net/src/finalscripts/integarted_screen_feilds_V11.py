    ...
    try:
        ...
        if metadata != []:
            try:
                ...
                if db.screen_data.insert_many(Remove(metadata)):
                    print("inserted")
        ...
        with open("LOC_screen_field_report" + '.csv', 'w', newline="") as output_file:
    ...
    try:
        ...
        if label_name_3 == [] and for_control_value == "":
            ...
            with open(filename + '.vb', 'r') as vb_file:
                ...
                for_control_reg1 = '\s*' + label_name + '\.Text\s*=\s*".*"'
                for lines in vb_file.readlines():
                    if lines.strip() == "":
                        continue
                                    ...
                                    for l in lookup_fun_store:
                                        ...
                                        if singleline_formation_flag:
                                            ...
                                            if not split_list[0].lower().__contains__('nothing'):
                                                ...
                                                print(final_dropdown_vaues_list[0].split(",")[0].replace("1.)", ''))
...
def fetch_stored_values_drop_down1(VB_PATH, VBpath_funtion_name, key_list):
    ...
    try:
        ...
        if q_key_dict == {}:
            return {"NoData":"Manual Intervention Required"}
        else:
            return q_key_dict
    ...
    with open(filename + '.vb', 'r') as vb_file:
        for line in vb_file.readlines():
            if line.strip().startswith("'"):
                continue
            if line.__contains__(VBpath_class_var.strip() + " As ") and line.__contains__("BusinessServices")and (not line.__contains__("nothing")):
                temp_path = line.split("As")[-1].replace("BusinessServices", "ApplicationAssembles").replace("New","").replace(".","\\").strip() + ".VB"
                path_list = temp_path.split("\\")
                path_list.insert(2, path_list[1] + "." + path_list[0].replace("ApplicationAssembles", "BusinessRules"))
                VBfile_path = "\\".join(path_list)
                return VBfile_path
