        ...
        for file in file_list:
            ...
            for line in f.readlines():
                ...
                if matches2 != []:
                    ...
                    op_json["component_name"] = file.split("\\")[-1].split(".")[0]
                    op_json['component_type'] = file.split("\\")[-1].split(".")[1].replace("aspx", "aspx.vb").upper()
                    op_json['calling_app_name'] = file.split("\\")[-2].replace(".BusinessRules", '').upper()
                ...
                if line.__contains__("Response.Redirect"):
                    ...
                    op_json['component_type'] = file.split("\\")[-1].split(".")[1].replace("aspx", "aspx.vb")
                    op_json['calling_app_name'] = file.split("\\")[-2].replace(".BusinessRules", '').upper()
                    op_json['called_name'] = line.split("(")[1].split(")")[0].replace('"',"").replace("'","").strip()
                    op_json['called_type'] = "Aspx"
...
pubShLokUp.db_delete("Vb_net1","cross_reference_report")
pubShLokUp.db_update("Vb_net1","cross_reference_report",xref.remove_duplicates(META_DATA))
