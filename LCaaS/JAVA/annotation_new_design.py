...
class Annotate:
    ...
    def flow_chart_keyword_annotation(self, db_name, flow_col, keyword_col,destn_col_name):
        flow_collection = self.Db_conn(db_name, flow_col)

        flow_cursy = flow_collection.find({"type": {"$ne": "metadata"}}, {'_id': False})
        flow_objects = self.fetch_db_cursor_to_list(flow_cursy)
        ...
        if keyword_col == "glossary":

            src = "Variable"
            dest = "Business_Meaning"

        elif keyword_col == "keyword_lookup":
            src = "keyword"
            dest = "Meaning"



        for flo_i in flow_objects:
            for dict in keywords_objects:
                if dict['keyword'] == "START":
                    replaced1 = re.sub(dict[src] + " ", " " + dict[dest] + " ", flo_i['option'][17:],
                                       flags=re.IGNORECASE)
                    flo_i['option'] = flo_i['option'][:17] + replaced1
                    # print(flo_i['option'])

                else:
                    replaced1 = re.sub(dict[src] + " ", " " + dict[dest] + " ", flo_i['option'],
                                       flags=re.IGNORECASE)
                    flo_i['option'] = replaced1
        self.db_delete(db_name, destn_col_name)
        self.db_update(db_name, destn_col_name, flow_objects)
...
Excel_path = "D:\\Keywords\\NAT_KEY.xlsx"
...
'''
annotate.keyword_load(Excel_path,'BNSF_NAT_POC_3','keyword_lookup')
#
...
"""Flowchart annotations for var """
annotate.flow_chart_keyword_annotation("BNSF_NAT","para_flowchart_data","glossary","translated_flowchart")

"""Flowchart annotations for keywords """
annotate.flow_chart_keyword_annotation("BNSF_NAT","translated_flowchart","keyword_lookup","translated_flowchart")
