...
def chartsAPI():
    ...
    try:
        time_largest_apps_start = timeit.default_timer()# Processing the list of largest applications
...
def rulesChartAPI():
    ...
    a = list(db.rule_report.aggregate([{"$group": {"_id": {"$toLower": "$rule_category"},
                                                   "count": {"$sum": 1}}},
                                       {"$group": {"_id": "null",
                                                   "counts": {"$push": {"k": "$_id",
                                                                        "v": "$count"}}}},
...
def applicationList():
    ...
    try:
        if flag == "def":
            ...
            list_of_programs = db.validation_report.distinct("application")
        # elif flag =="xref":
        #     list_of_programs = db.rule_report.distinct("calling_app_name") 
        # elif flag =="form":
        #     list_of_programs = db.rule_report.distinct("calling_app_name")            
        else:
            list_of_programs = db.master_inventory_report.distinct("application")
        
        applist = sorted(list(set([item for item in list_of_programs if item.strip()!=""])))
...
def masterInventory():
    ...
    try:
        if appname == "All":
            data_object['data'] = list(db.master_inventory_report.find({'type': {"$ne": "metadata"}}, {'_id': 0}))
        else:
            data_object['data'] = list(db.master_inventory_report.find({"application": appname}, {'_id': 0}))
...
def crossReference():
    ...
    try:
        ...
        data_object['total_record_count'] = db.cross_reference_report.count_documents({'type': {"$ne": "metadata"}})
        metadata = db.cross_reference_report.find_one({"type": "metadata"}, {'_id': 0})
        data_object['headers'] = metadata['headers']
            
        if appname == "All":
            cursor = db.cross_reference_report.find({'type': {"$ne": "metadata"}}, {'_id': 0})
            dfDataset = pd.DataFrame.from_records(cursor)# Pandas code to convert cursor to a dict
            ...
            if override_filter == 'yes':
                ...
                if search_filter:
                    ...
                    cursor = db.cross_reference_report.find({"calling_app_name": appname}, {'_id': 0}).limit(CROSS_REFERENCE_LIMIT)
                    dfDataset = pd.DataFrame.from_records(cursor)
                    data_object['data'] = dfDataset.fillna("").to_dict('records')
...
def CRUD():
    ...
    try:
        if appname == "All":
            data_object['data'] = list(db.crud_report.find({'type': {"$ne": "metadata"}}, {'_id': 0}))
        else:
            data_object['data'] = list(db.crud_report.find({"calling_app_name": appname}, {'_id': 0}))
...
def uploadglossary():
    ...
    for json_dict in temp_file:
        ...
        try:
            db.glossary.update({"File_name": json_dict["File_name"],
                                "Variable": json_dict["Variable"]},
                               {"$set": {"Business_Meaning": json_dict["Business_Meaning"]}})
        except:
...
def spiderTypes():
    ...
    try:
        distinct_types = []
        if apname == "All":
            component_types = list(db.cross_reference_report.distinct("component_type"))
            called_types = list(db.cross_reference_report.distinct("called_type"))
        else:
            component_types = list(db.cross_reference_report.distinct("component_type", {"calling_app_name": apname}))
            called_types = list(db.cross_reference_report.distinct("called_type", {"calling_app_name": apname}))
        distinct_types = set(component_types + called_types)
...
def spiderList():
    try:
        ...
        distinct_elements = []
        if apname == "All" and option is not None:
            calling_names = list(db.cross_reference_report.distinct("component_name", {"component_type": option}))
            called_names = list(db.cross_reference_report.distinct("called_name", {"called_type": option}))
            distinct_elements = set(calling_names + called_names)
            return jsonify(sorted(list(distinct_elements)))

        elif apname != "All" and option is not None:
            calling_names = list(db.cross_reference_report.distinct("component_name", {"component_type": option,
...
def spiderFilterList():
    ...
    not_to_list = list(db.missing_component_report.find({"type": {"$ne": "metadata"}}))
    response = {}
    try:
        ...
        if response['component_name'] is not None:
            try:
                connections = list(db.cross_reference_report.find(
                    {"$or": [{"$and": [{"component_name": response['component_name']},
                                       {"component_type": response['component_type']}]},
                             {"$and": [{"called_name": response['component_name']},
                                       {"called_type": response['component_type']}]}]},{'_id': 0}))
...
def procedureAppList():
    ...
    try:
        if flag == "ppf":
            appln_name_set = db.procedure_flow_table.distinct("application")
        elif flag == "bre":
            appln_name_set = db.rule_report.distinct("application")
        else:
            appln_name_set = db.master_inventory_report.distinct("application")

        appln_name_set = [item for item in appln_name_set if item.strip()!=""]
        return jsonify({"application_list": sorted(list(set(appln_name_set)))})
    except Exception as e:
        print('Error:', str(e))
        return jsonify({"status": "failure", "reason": "Error: " + str(e)})
...
def procedureFlowCompList():
    ...
    try:
        if option is not None:
            if flag=="ppf":
                component_list = list(db.procedure_flow_table.find({"application": option},
                                                                      {'_id': 0, "component_name": 1}))
                component_list = [item['component_name'] for item in component_list]
            elif flag=="bre":
                component_list = list(db.rule_report.find({"application": option},
                                                                      {'_id': 0, "pgm_name": 1}))
                component_list = [item['pgm_name'] for item in component_list]
            else:
                component_list = list(db.master_inventory_report.find({"application": option},
                                                                      {'_id': 0, "component_name": 1}))
                component_list = [item['component_name'] for item in component_list]        
    
        return jsonify({"component_list": sorted(list(set(component_list))), "application_name": option})
    except Exception as e:
        print("Error", str(e))
        return jsonify({"status": "failure", "reason": "Error: " + str(e)})
...
def updateBRE():
    try:
        ...
        if db.rule_report.update_one({"fragment_id": fragment_id}, {
            "$set": {
...
def comment_lines():
    ...
    ctype = request.args.get("component_type")
    cursor = db.cobol_output.find_one({"component_name": name, "component_type": ctype},{'_id': 0})
    cursor["commented_lines"] = cursor.pop("codeString")
    del cursor["application"]
...
def screen_simulation():
    ...
    cursor = db.screen_simulation.find({"component_name": name, "application": appname},{'_id': 0})
    cursor["simulation"] = cursor.pop("codeString")
    del cursor["component_type"]
    return jsonify(cursor)
...
def codeString():
    ...
    #     mapName = mapName.split("(")[0]+".jsp"
    #     print(mapName)
    # cursor = db.screen_simulation.find({"component_name": mapName},{'_id': 0})
    # print(type(cursor))
    # cursor = {"codeString": cursor['codeString']}
    # return jsonify(cursor)
...
def missingComponents():
    ...
    try:
        if appname == "All":
            data_object['data'] = list(db.missing_components_report.find({'type': {"$ne": "metadata"}}, {'_id': 0}))
        else:
            data_object['data'] = list(db.missing_components_report.find({"application": appname}, {'_id': 0}))
...
def deadparalist():
    ...
    headers = ["component_name", "component_type", "application",
                "dead_para_list", "dead_para_count", "no_of_dead_lines"]
    try:
        if appname == "All":
            datavalue["data"] = list(db.master_inventory_report.find({"type": {"$ne": "metadata"},
                                                      "$or": [{"component_type": "SERVLET"},
                                                              {"component_type": "JAVA_CLASS"},
                                                              {"component_type": "INTERFACE"},
                                                              {"component_type": "DATA_ACCESS_OBJECT"}],
                                                      "dead_para_count": {"$gt": 0}},
                                                     {"component_name": 1, "component_type": 1,
                                                      "application": appname, "dead_para_list": 1,
        ...
        # datavalue["headers"] = metadata["headers"]      
    except:
        print("")
...
def MissingAppList():
    applist = []
    try:
        list_of_programs = list(db.missing_components_report.distinct("application"))
        applist = sorted(list(set(list_of_programs)))
        applist.insert(0, "All")
        return jsonify({"application_list": applist})
    except Exception as e:
        print('Error:', str(e))
...
def dropAppList():
    try:
        list_of_programs = list(db.drop_impact.distinct("application"))
        applist = sorted(list(set(list_of_programs)))
        applist.insert(0, "All")
        return jsonify({"application_list": applist})
    except Exception as e:
...
def nameList():
    ...
    if flag == "def":
        if selectedApplication == "All":
            ...
            cursor = db.screenfields.find({'type': {"$ne": "metadata"},"application": selectedApplication},
                                          {"filename": 1, '_id': 0})
        output = [document["filename"] for document in cursor]  
    elif flag == "val":
        if selectedApplication == "All":
            cursor = db.validation_report.find({'type': {"$ne": "metadata"}}, {"filename": 1, '_id': 0})
        else:
            cursor = db.validation_report.find({'type': {"$ne": "metadata"},"application": selectedApplication},
                                          {"filename": 1, '_id': 0})
        output = [document["filename"] for document in cursor]  
    else:
        if selectedApplication == "All":
            cursor = db.master_inventory_report.find({'type': {"$ne": "metadata"}}, {"component_name": 1, '_id': 0})
        else:
            cursor = db.master_inventory_report.find({'type': {"$ne": "metadata"}, "application": selectedApplication},
                                                     {"component_name": 1, '_id': 0})
        output = [document["component_name"] for document in cursor]    
...
def validationreport():
    ...
    try:
        if apname == "All":
            data_object['data'] = list(db.validation_report.find({"filename": filename},{"_id": 0}))
        else:
            data_object['data'] = list(db.validation_report.find({"application": apname, "filename": filename},{"_id": 0}))
