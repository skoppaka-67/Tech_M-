    ...
    for component in compnentname_list:
     if component != None:
        ...
        for iter in matadata:
            try:
                iter['Rule'] = parent_rule_id_dict[iter['Rule']]
                # iter["parent_rule_id"] = str(iter["parent_rule_id"]).strip("[]").replace("'", "")


                if len(iter['rule_relation']) == 1:

                    iter['rule_relation'] = parent_rule_id_dict[iter['rule_relation'][0]]
                else:
                    ...
                    iter["rule_relation"] = ",".join(iter["rule_relation"])
            except KeyError as e:
                print(iter)
