...
db = client["TestAutomation"]  # Creating a Collection
            ...
            if os.path.isdir(appPath):
                if os.path.isdir(scenarioPath):
                    if os.path.exists(scenarioPath + "\\Batchfiles\\Test_batch.bat"):
                        ...
                        conf_log = conf_log_modification()
...
def create_CombinedMultiScenarioFile_SameFile(json_format, testRunName, flag):
    ...
    for connection in connections:
        if connection["test_name"] == testRunName:
            ...
            if os.path.exists(mulipleScenarioPath + "\\combined_multi_scenario_file.js"):  # check whether the
...
def uploadExcelRun():
    # folderName = request.args.get("folderName")
    # import pandas as pd
    global mulipleScenarioPath  # declare muipleScenarioPath as global
    file = request.get_data()  # get value in the form of post method
    final_val = []
    str_file = file.decode("utf-8")  # decode it with utf-8
    temp_List = []
    sheetLength = 0
    print("str_file", str_file)
    json_format = json.loads(str_file)  # load the json
    print("json_format", json_format)

    # Checking the muliple scenario file exist or not, if exists remove the file
    testRunName = request.args.get("testName")  # get the
    flag = "Upload"
    final_js = create_CombinedMultiScenarioFile_SameFile(json_format, testRunName, flag)
        ...
        if connection[
            ...
            if os.path.isdir(filePath):
                if os.path.exists(filePath + "\\spec.js"):  # check if the file exists or not
                    ...
                    new_spec = provideJasmineSpecReporter() + str(
                        replace_spec) + "});"  # create a file structure with required imports and values
                    if os.stat(
                            appPath + "\\combined_multi_scenario_file.js").st_size == 0:  # represents the size of the file in bytes
                        new_combined_file = provideShortSpec() + str(replace_spec)
...
def multipletestReportGenerationRun(scenarioPath):
    # folderName = request.args.get("folderName")
    print(scenarioPath)
    open_file = open(scenarioPath + "\\multiple_conf_file.log")  # open log file
    log_file = open(scenarioPath + "\\multiple_conf_file.log").read()  # read the log file
...
def conf_log_modification_show():
    try:
        ...
        final_report['headers'] = ['Date', 'Time', 'Application Name', 'Scenario Name',
                                   ...
                                   'WebDriver Instance', 'View/Edit']

        # final_list.append(b)
        final_report['data'] = full_dict
...
@app.route(
    "/scheduledJobList")  # View all the scheduled job list, gets called when browser compatablility test button is clicked
...
def runSavedTestRun():
    ...
    scenario_file = open(base_location + "\\testRun\\" + testRunName + "\\" + "spec.js", "w+")
    ...
    script1 = '''START /B cmd "cd ''' + base_location + '''\\" && protractor ''' + base_location + \
              ...
              "testRun" + '\\' + testRunName + '''\\multiple_conf_file.log'''
    ...
    for scene_app in split_scenario:
        ...
        if os.path.isdir(path_var) == False:
            ...
            specFile = open(path_var + "\\spec.js").readlines()
            print("spc",specFile)
            for idx, lines in enumerate(specFile):
                if flag == 1:
                    if idx == len(specFile) - 1:
                        # requiredLines += "});"
                        flag = 0
                        break
                    requiredLines += lines
    ...
    log_file = open(base_location + '\\' + "testRun" + '\\' + testRunName + '\\multiple_conf_file.log').read()
...
def getMetricsDetailsforTestJobDoc(jobName):
  ...
  '''
    ...
    confLog = open(path + "\\multiple_conf_file.log").readlines()
...
def getLogsAndScreenshotsforTestJobDoc(jobName, screenshotFlag):
  ...
  '''
    ...
    conf_log = open(appPath + "\\multiple_conf_file.log", "r").readlines()
                        ...
                        if os.path.isdir(appPath):
                            if os.path.isdir(scenarioPath):
                                for img in os.listdir(scenarioPath):
                                    if img.endswith(".png"):
                                        fileName.append(img)
                                        shutil.copy(scenarioPath + "\\" + img, destination_folder)
    ...
    try:
        ...
        for filename in glob.glob(os.path.join(base_location + '\\' + outerFolderName + '\\' + folderName, '*.png')):
            if filename.split("\\")[-1].startswith("page") and filename.split("\\")[-1].endswith(".png"):
                num = filename.split("\\")[-1].split("page")[1].split(".png")[0]
        ...
        for line in buf:
            ...
            # insert_line+=line
            if line.__contains__("element") and line.__contains__("click"):
                insert_line += line.replace(line, new_line)
                counter += 1
            elif line.__contains__("element") and line.__contains__("sendKeys"):
                insert_line += line.replace(line, new_line)
                counter += 1
            insert_line += line
        ...
        return jsonify(e)
def xpathExtract():
    #newfile=open(base_location + "\\" + outerFolderName + "\\" + folderName + "\\" + "new_file.txt").read()
    newfile=open(r"D:\Recorded_Scenarios\natgeo\href1\new_file.txt").read()
    splitfile=newfile.split(",")
    xpath_list=[line.split()[2] for line in splitfile if not line.__contains__("Open Browser")]
    print(xpath_list)
    return xpath_list
        ...
        for idx, line in enumerate(splitted_robocorder):
            if flag == 0:
                ...
                if line.lower().__contains__("click") and not line.lower().__contains__("undefined") and not line.__contains__("nonIndexBased") and not line.__contains__("indexBased") and not line.__contains__("existingXpath") and not line.__contains__("XpathWithVal"):
                    ...
                    # if assertxpath.__contains__("//input"):
                    #     method="getAttribute('value')"
                    # if assertcondition == "Presence":
                    #     method = "isPresent()"
                    # if assertcondition=="Contains" :
                    #     method="getAttribute('value')"
...
def assertionDataTable():
    try:
        ...
        print("lasstttt", folderName)
        # shutil.copy(download_location,
        #             base_location + "\\" + outerFolderName + "\\" + folderName + "\\" + "new_file.txt")  # copy the new_file.txt file from downloads folder to the recorded test case folder
        f_open = open(base_location + "\\" + outerFolderName + "\\" + newfolderName + "\\new_file.txt",
                      "r")  # open the new_file.txt in read mode
        content = f_open.read()  # read the new_file.txt
        xpath = []  # initialize empty list for storing xpath.
        original_value = []  # initialize empty list for storing value.
        line_numers = []  # initialize empty list to store the line number of the line in new_file.txt.
        split_file = content.split(",")  # split the new_file.txt content using comma .

        for idx, line in enumerate(split_file):  # iterate through the lines splited using comma, along with line index idx.
            ...
            if line.split()[-1].__contains__("//") == False and line.__contains__("Open Browser") == False:
                ...
                line_numers.append(idx + 1)  # append the line number

        outDict = {}  # initialize a dictionary
        finalList = []  # initialize a list
        finalDict = {}  # initialize a final json to return
        ...
        for val in assertConditionDropDown:
            assertDropDownValue.append("<option>" + val + "</option>")
        for (path, value, line_numer) in zip(xpath, original_value,
                                             line_numers):  # iterate through xpath , original_value and line_numers lists.
            outDict["xpath"] = path  # assign the xpath to outDict dictionary for key as xpath
            outDict["value"] = value  # assign the value to outDict dictionary for key as value
            outDict["index"] = ""
            outDict["operation"] = assertConditionDropDown
            outDict["Assertion Value"] = Remove(xpathdropdown)  # assign the empty value to outDict dictionary for key as asserted value
            outDict["stepNumber"] = line_numer  # assign the line_numer obtained to outDict dictionary for key as stepNumber
            finalList.append(outDict)  # in finalList append the whole outDict dictionary.
            outDict = {}  # empty the outDict for each iteration and continue the process.

        finalDict['headers'] = ["checkbox", "stepNumber", "xpath","index", "value","operation", "Assertion Value",
                                "Add/Remove"]  # add the headers as a key value pair
        finalDict["data"] = finalList  # add the finalList as a key value pair.
        ...
        # db.assertTable.insert_many(outputList)  # insert the finalList into the db.
        # for l in finalList:
        #     if "_id" in l.keys():
        #         del l["_id"]  # delete the object id from the finalList
        # print("final list", finalList)
        print("final dict", finalDict)
        return finalDict  # return the finalDict to UI
    except Exception as e:
        return jsonify(e)  # convert exception into json
def assertPage(): # for alternate value data accumulation in table
    try:
        import glob
        import os, shutil
        # f = open(base_location + "\\" + outerFolderName + "\\" + folderName + "\\" + "new_file.txt",
        #          "w+")  # open the new_file.txt in write mode
        # f.close()
        shutil.copy(download_location,
                    base_location + "\\" + outerFolderName + "\\" + folderName + "\\" + "new_file.txt")  # copy the new_file.txt file from downloads folder to the recorded test case folder
        f_open = open(base_location + "\\" + outerFolderName + "\\" + folderName + "\\new_file.txt",
                      "r")  # open the new_file.txt in read mode
        # index = request.args.get("index")  # get the index value from UI
        # print("index value", index)
        content = f_open.read()  # read the new_file.txt
        # initialize a list with all conditions available
        xpath = []  # initialize empty list for storing xpath.
        original_value = []  # initialize empty list for storing value.
        line_numers = []  # initialize empty list to store the line number of the line in new_file.txt.
        split_file = content.split(",")  # split the new_file.txt content using comma .
        ...
        for idx, line in enumerate(split_file):  # iterate through the lines splited using comma, along with line index idx.
            ...
            if line.split()[-1].__contains__("//") == False and line.__contains__("Open Browser") == False:
                xpath.append(line.split()[2])  # split and append xpath in list
                original_value.append(line.split(xpath[-1])[-1].strip())  # split and append original value in list
                line_numers.append(idx + 1)  # append the line number
        outDict = {}  # initialize a dictionary
        finalList = []  # initialize a list
        finalDict = {}  # initialize a final json to return
        for (path, value, line_numer) in zip(xpath, original_value,
            ...
            outDict["xpath"] = path  # assign the xpath to outDict dictionary for key as xpath
            outDict["value"] = value  # assign the value to outDict dictionary for key as value
            outDict["Alternative Value"] = ""  # assign the empty value to outDict dictionary for key as asserted value
            outDict[
                "stepNumber"] = line_numer  # assign the line_numer obtained to outDict dictionary for key as stepNumber
            finalList.append(outDict)  # in finalList append the whole outDict dictionary.
            outDict = {}  # empty the outDict for each iteration and continue the process.
...
def conf_log_modification():
  try:
    ...
    open_file = open(base_location + "\\" + outerFolderName + "\\" + folderName + "\\" + "conf.log",
    ...
    final_report['headers'] = ['Date', 'Time', 'Application Name', 'Scenario Name',
                               'Error lines in spec','Error Message', 'Status', 'Time in Seconds',
                               'WebDriver Instance', 'View/Edit']  # assign necessary headers
...
@app.route("/insertNewRow")  # When user clicks on Add new row in Assert Table
def insertNewRow():
    try:
        ...
        for step_number, xpath_list, value_list, assert_list in zip(stepNumber, xpath_list_row,
            ...
            index_value += 1
            # print("index ", index)
            # print("index_value",index_value,len(xpath_list_row))
            #
            # print("NNNNNN", newindex, index_value)
            # print("LLLL",newindex,lastindex)
...
@app.route("/deleteRow")  # To delete a row in assert table
def deleteRow():
    try:
        ...
        for step_number, xpath_list, value_list, assert_list in zip(duplicate_step_number,
            ...
            # print(assert_list_row[5],"xpathval")
            # print(assert_list_row[6], "xpathvallll")
            xpath_list=xpath_list.replace("HASH","#")
...
def runSavedTestRun1(testRunName):
    ...
    fulString = provideShortSpec() # multiline string protractor code
    # scenarios="Deposit_Flow[XYZ_BANK], Bank_flow[XYZ_Bank]"
    ...
    scenario_file = open(base_location + "\\testRun\\" + testRunName + "\\" + testRunName + ".js",
    ...
    conf_string = '''
     ...
     '''  # multiline string, configuration file contents
    conf_file.write(conf_string)  # write configuration content in conf_file
    ...
    for scene_app in split_scenario:  # iterate through list of test scenarios
        ...
        if os.path.isdir(path_var) == False:  # if path_var path not available
            ...
            specFile = open(path_var + "\\spec.js").readlines()  # open spec.js file and read
            for lines in specFile:  # iterate through spec.js file
                if flag == 1:
                    requiredLines += lines  # add line to requiredLines variable

                if lines.strip().startswith("it("):  # if line starts with 'it'
                    flag = 1
                    requiredLines += "it(" + lines.split("it(")[1].replace("1", str(testCount))  # replace testCount

                if lines.__contains__("extract();"):  # if line contains extract
                    requiredLines += "});"  # add }); to variable
                    flag = 0
                    break  # exit loop
        ...
        if connection["test_name"] == testRunName:  # if testrunname present in db
            ...
            if os.path.isdir(filePath):  # if filepath available
                if os.path.exists(filePath + "\\spec.js"):  # if spec.js available in the path
                    ...
                    new_spec = provideShortSpec() + str(
                        replace_spec) #+ "});"  # add lines added with semicolon
           ...
           '''
                    ...
                    multipleBatchFile = "START /B cmd " + appPath + " && protractor " + appPath + "\\" + schedulerName + "\\multiple_conf_file.js >" + appPath + "\\" + schedulerName + "\\multiple_conf_file.log"  # path for batch file
                    comb_batch_file_data = "START /B cmd " + appPath + " && protractor " + appPath + "\\" + schedulerName + "\\combined_multi_scenario_file.js >" + appPath + "\\" + schedulerName + "\\multiple_conf_file.log"  # path for batch file
                            ...
                            for line in sec_test.split(";"):
                                for idx, header in enumerate(head_list):
                                    if (
                                        ...
                                        break
                                replace_spec += line + ";"
                        new_spec =provideShortSpec() + str(replace_spec)
                        new_conf = '''
                    exports.config = {
     framework: 'jasmine',
     seleniumAddress: 'http://localhost:4444/wd/hub',
...
@app.route("/piechartData")  # schedule the test job either daily or specific time
def piechartData():
    count = 0
    count1 = 0
    total=0
    ...
    scenario_name = request.args.get("scenarioname")
    print("appname from piechart",application_name,scenario_name)
    outerFolderName = application_name
    folderName = scenario_name

    if os.path.exists(base_location+"\\"+application_name+"\\"+scenario_name+"\\"+"new_conf_log.log"):

        f=open(base_location+"\\"+application_name+"\\"+scenario_name+"\\"+"new_conf_log.log","r")

        for line in f.readlines():
            if line.strip().startswith("'Status': 'passed'"):
                count+=1
            elif line.strip().startswith("'Status': 'failed'"):
                count1+=1
        total=count+count1
        passed=(count/total)*100
        failed=(count1/total)*100
        output["passed"]=passed
        output["failed"]=failed
        print(output)
        return output
def provideShortSpec():
...
def provideJasmineSpecReporter():
    spec='''const jsdom = require("jsdom");
                         const fs = require('fs');
                         const path = require('path');
