...
def create_CombinedMultiScenarioFile_SameFile(json_format, testRunName, flag):
    ...
    log_filename = "multiple_conf_file.log"
    conf_log = conf_log_modification(log_filename)
    output_json = {"status": "Success", "conf_log": conf_log}
...
def uploadExcelRun():
    ...
    final_val = []
    str_file = file.decode("utf8")  # decode it with utf-8
    temp_List = []
    sheetLength = 0
    ...
    moveReport("testRun",testRunName)
...
def runSavedTestRun():
    ...
    # outerFolder="testRun"
    # conf_string = provideConf(spec_file_name,base_location,outerFolder,folderName)
    ...
    #                 requiredLines += lines
    #
    #             # if lines.__contains__("describe(") and lines.__contains__("it("):
    #             #     flag = 1
    #             #     requiredLines += "it(" + lines.split("it(")[1].replace("1", str(testCount))
    #
    #             if lines.__contains__("it("):
    #                 flag = 1
    #                 requiredLines += lines
    #     print("req LIness hvgd HHHHH",requiredLines)
    # fulString += requiredLines + "});"
    # print("fulstring starts",fulString)
    # scenario_file.write(fulString)
    # scenario_file.close()
    writestring=''
    open_spec=open(base_location + '\\' + "testRun" + '\\' + testRunName + '\\'+testRunName+'.js',"r").readlines()
    open_new_spec=open(base_location + '\\' + "testRun" + '\\' + testRunName + '\\spec.js',"w+")
    for idx,lines in enumerate(open_spec):
        if lines.__contains__("browser.sleep("+str(sleep_delay)+");});") or (lines.__contains__("browser.sleep(") and lines.__contains__("});")):
            lines=lines.split("});")[0]
            print(lines)
        writestring+=lines
    open_new_spec.write(writestring)
    open_new_spec.close()
    ...
    for collection in collections:
        if collection["test_name"] == testRun:
            ...
            for app_scene in split_scenario:
                ...
                connections = db.folderName.find()
                finalJson = {}
                print("application name and scenario name",application_name,scenario_name)
                        ...
                        if os.path.isdir(appPath):
                            if os.path.isdir(scenarioPath):
                                for img in os.listdir(scenarioPath):
                                    if img.endswith(".png"):
                                        count = count + 1
                                        image_name="run"+str(count)+".png"
                                        fileName.append(image_name)
                                        shutil.copy(scenarioPath + "\\" + img, destination_folder)

                                        try:
                                            os.rename(destination_folder + "\\" + img,destination_folder+ "\\" + "run"+str(count)+".png")
                                        except FileExistsError as e:
                                            print("Screenshots already moved ")
					 ...
					 if (result.status == 'failed') {
						 browser.getCapabilities().then(function (caps) {
							...
							var stream = fs.createWriteStream(`''' + base_location.replace("\\",
                                                                                           "\\\\") + '''\\\\''' + outerFolderName + '''\\\\''' + folderName + '''\\\\screenshots`+ browserName + '-' + result.fullName+ '.png');
