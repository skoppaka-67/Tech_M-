...
file_location = "D:\\BNSF\\POC\\"
# Specify the file name
file_name = "Natural_Components.xlsx"
...
def updateCrudAPPNAME_CALLING():
    ...
    try:
        for ite in range(2, sh.max_row):
            print(sh.cell(row=ite, column=1).value, sh.cell(row=ite, column=3).value)
            if db.crud_report.update_many({"component_name": sh.cell(row=ite, column=1).value}, {
                "$set": {"calling_app_name": sh.cell(row=ite, column=3).value}}).acknowledged:
                #sh.cell(row=ite, column=5).value = 'UPDATED'
                print('CALLING_APP =====>Updated', sh.cell(row=ite, column=1).value + '\'s application as',
                      sh.cell(row=ite, column=3).value)
...
def updateBRE():
    ...
    try:
        for ite in range(2, sh.max_row):
            ...
            row = db.bre_rules_report.update_many({"pgm_name": component_name},{"$set": {"application": application_name}})
...
def update_orphan():
    ...
    try:
        for ite in range(2, sh.max_row):
            ...
            row = db.orphan_report.update_many({"component_name": component_name},{"$set": {"application": application_name}})
            print(component_name,application_name)
        time_update_minv_stop=timeit.default_timer()
        time_taken = time_update_minv_stop - time_update_minv_start
        print('Time taken :',time_taken)
...
def update_dropImpact():
    ...
    try:
        for ite in range(2, sh.max_row):
            ...
            row = db.drop_impact.update_many({"drop_impact_name": component_name},{"$set": {"application": application_name}})
            print(component_name,application_name)
        time_update_minv_stop=timeit.default_timer()
        time_taken = time_update_minv_stop - time_update_minv_start
        print('Time taken :',time_taken)
