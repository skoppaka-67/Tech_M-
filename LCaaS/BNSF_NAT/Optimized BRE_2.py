            ...
            try:
                if j < len(bre_json_objects):
                    ...
                    if bre_json_objects[i]['parent_rule_id'] != bre_json_objects[j]['parent_rule_id']:
                        ...
                        if len(storage) > 0 and storage[-1] == bre_json_objects[i]['source_statements']  + " <br> ":

                            new_json['source_statements'] =new_json[
                                'source_statements'] + " <br> " + str(storage).strip(
                                "[]").replace(",", "").replace('"', "")
                        else:
                            if bre_json_objects[i]['source_statements'].strip().startswith("IF"):
                                new_json['source_statements'] = bre_json_objects[i]['source_statements']+ "Then" + new_json[
                                    'source_statements'] + " <br> " + str(storage).strip(
                                    "[]").replace(",", "").replace('"', "")
                            else:
                                new_json['source_statements'] = bre_json_objects[i]['source_statements']+new_json['source_statements'] + " <br> " + str(storage).strip(
                                    "[]").replace(",", "").replace('"', "")
                    ...
                    if bre_json_objects[i]['parent_rule_id'] == bre_json_objects[j]['parent_rule_id']:
                        ...
                        new_json['pgm_name'] = bre_json_objects[i]['pgm_name'].replace(".","")
                        if bre_json_objects[i]['source_statements'].strip().startswith("IF"):
                            new_json['source_statements'] = bre_json_objects[i]['source_statements'] + 'Then'+" <br> " + \
                                                            bre_json_objects[j]['source_statements']
                        else:
