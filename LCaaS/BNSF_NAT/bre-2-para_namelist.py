...
SCRIPT_VERSION = "tagging Ecaluate to first whens rule id,first occurence of paraname will be captured insted of last, paranames will captured as list "
'''Retained single quotes in source statements '''
mongoClient = MongoClient('localhost', 27017)
db = mongoClient['bnsf1']
...
def main():
    ...
    for component in compnentname_list:
        ...
        if component in ignore_list  or component in bre_complist:
        ...
        for i in range(1, sheet.nrows):
            ...
            try:
                if j < sheet.nrows:
                    ...
                    if sheet.cell_value(i, 8) == sheet.cell_value(j, 8):
                        # new_json.clear()
                        new_json["para_name"] = para_storage.append(sheet.row_values(i)[4])
                        new_json['pgm_name'] = sheet.row_values(i)[2].replace(".","")
