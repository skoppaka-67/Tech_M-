...
# def usernameValidation():
    ...
    #         # output={"status":"Success","session":session}
...
def logout():
    ...
    loggedout_time= datetime.datetime.now()
    traceDict = {
        "username": username,
        # "ip": ip,
        "Login Time": loggedout_time
    }
    db.logoutTrace.insert(traceDict)
    # collection=db.LoginCred.delete_one({"username":username, "ip":ip})
    for data in db.LoginCred.find({"username":username}):
        print(data)
        db.LoginCred.delete_one(data)
