...
from flask import Flask, request, jsonify, flash, redirect, url_for, render_template,make_response
...
# cors = CORS(app, resources={r"/api/vi/*": {"origins": "http://localhost:4200"}})
CORS(app)
...
def usernameValidation1():
    ...
    #         if len(session) > 0 and session['username'] == username:
    #             return {"status":"already logged in"}
    #         session['username'] = username
    #         session['password'] = password
    #         token = jwt.encode({'username': username , 'exp': datetime.datetime.utcnow() + datetime.timedelta(minutes=15)},
    ...
    user=request.get_data()
    value=user.decode('utf-8')
    loggedin_time=datetime.datetime.now()
    ...
    db.LoginTrace.insert(traceDict)
    collections=db.LoginCred.find({"username":username, "password":password})
    print(collections.count())
    if collections.count()>0:
        return jsonify({"loggedin":"Fail"})
    else:
...
@app.before_request
def decorated():
        # cook = request.cookies.get("isLoggedIn")
        # print(cook)

        if request.endpoint == "logout":
            session.clear()
            return "success"
        if request.endpoint != "usernameValidation":
            token = None
        ...
        elif  request.endpoint == "usernameValidation" and request.method == 'POST' :
               print("redirect")
               return usernameValidation1()
...
#     cursor = db.master_inventory_report.find({"type":{"$ne":"metadata"},"component_type":"NATURAL-PROGRAM","dead_para_count":{"$gt":0}},{"component_name":1,"component_type":1,"dead_para_list":1,"dead_para_count":1,"no_of_dead_lines":1,"_id":0})
...
def glossary():
    ...
    try:
        ...
        if override_filter == 'yes':
            ...
            data_object['displayed_count'] = cursor.count()
...
def uploadglossary():
        ...
        for json_dict in temp_file:
            ...
            try:
                #print( json_dict["component_name"],json_dict["Variable"],json_dict["Business_Meaning"])
                db.glossary.update_many({"component_name": json_dict["component_name"],"Variable":json_dict["Variable"]},{"$set":{
...
def getCode():
    ...
    if component_type == "MAP" or component_type == "BMS" :
        start = timeit.default_timer()
        cursy = db.codebase.find_one({"$and": [{'component_name': component_name.split(".")[0]+".MAP"}, {'component_type': "MAP"}]},
                                     {"_id": 0})
        end = timeit.default_timer()
        # cursy['completed_in']=end-start
        return jsonify(cursy)
    if component_type == "PDA" or component_type == "DDM" or component_type == "LDA" :
        ...
        start = timeit.default_timer()
        cursy = db.codebase.find_one({"$and":[{'component_name': component_name.split(".")[0]+".NAT"},{'component_type': component_type}]},{"_id":0})
        end = timeit.default_timer()
        #cursy['completed_in']=end-start
        return jsonify(cursy)
                    ...
                    for connection in connections:
                        if connection['called_name'].strip()!='':
                            ...
                            if connection['component_name'] == response['component_name']:
                                # findDup = {}
                                # findDup['source'] = connection['component_name']
                                # findDup['target'] = connection['called_name']
                                # findDup['label'] = 'C'
                                # print(findDup,'finddup')
                                # print(response['links'])
                                # if findDup not in response['links']:
                            ...
                            # If the CALLED component's name is same as option
                                ...
                                if connection['component_name'] != connection['called_name']:
                                    response['links'].append({"source": "p_" + connection['called_name'],
                                                              "target": "p_" + connection['component_name'], "label": "C"})
...
def procedureFlowCompList():
    ...
    if option is not None:
        ...
        try:
            response = db.master_inventory_report.find({"application": application_name, "component_type":"NATURAL-PROGRAM"})
                        ...
                        if connection['called_name'].strip() != '':
                            ...
                            if connection['component_name'].strip() == input_name:
                                if parent_child == "child":
                                    ...
                                    if connection['called_type'].upper() == 'NATURAL-PROGRAM':
...
def cycComplex():
    ...
    application = []
    orphan_data1 = []
    cursor = db.master_inventory_report.find({"component_type": "NATURAL-PROGRAM"})
    for document in cursor:
...
def inbboundOutbound():
    ...
    out_data.append(out_descending)
...
def brerule():
    ...
    print(search_filter, type(search_filter))
    if(override_filter=='yes'):
            ...
            if(connection['rule_category']==bre_connection):
                if (search_filter != '' ):
                    if(len(search_filter)>=3):
                        # print(search_filter)
                        # print("pgm_name",type(connection['pgm_name']))
                        # print(connection['source_statements'])
                        # print(connection['rule_category'])
                        # print(connection['fragment_Id'])
                        # print(connection['parent_rule_id'])
                        # print(connection['business_documentation'])
                        # print(connection['statement_group'])
                        # print(connection['para_name'], search_filter)
                        # print(str(connection['para_name']).find(search_filter))
                        if(connection['pgm_name'].find(search_filter)>=0 or str(connection['para_name']).find(search_filter)>=0 or connection['source_statements'].find(search_filter)>=0 or connection['rule_category'].find(search_filter)>=0 or connection['fragment_Id'].find(search_filter)>=0 or connection['parent_rule_id'].find(search_filter)>=0 or connection['business_documentation'].find(search_filter)>=0 or connection['statement_group'].find(search_filter)>=0):
...
def cicsfield():
    ...
    try:
        ...
        for document in cursor:
            #print(document)
            #del document['file_name']
            document["bms_name"] = document["bms_name"]
            data_object['data'].append(document)
        #print(data_object)
    except Exception as e:
        print('')
...
def missingComponents():
    ...
    if appname == "All":
        try:
            #cursor = db.missing_components_report.find({'type': {"$ne": "metadata"}}, {'_id': 0})
            cursor = db.missing_components_report.find({ "$and": [{'type': {"$ne": "metadata"}},{'component_type': {"$ne": "FILE"}}]},{"_id":0})
        ...
        try:
            ...
            cursor = db.missing_components_report.find(
                {"$and": [{"application": appname}, {'component_type': {"$ne": "FILE"}}]}, {"_id": 0})
            metadata = db.missing_components_report.find_one({"type": "metadata"}, {'_id': 0})
            data_object['headers'] = metadata['headers']
            # #print(cursor)
            for document in cursor:
    ...
    if appname == "All":
        try:
            ...
            cursor = db.master_inventory_report.find(
                {"type": {"$ne": "metadata"}, "component_type": "NATURAL-PROGRAM", "dead_para_count": {"$gt": 0}},
        ...
        try:
            headers = ["component_name", "component_type", "application", "dead_para_list", "dead_para_count",
                       "no_of_dead_lines"]
            cursor = db.master_inventory_report.find(
                {"type": {"$ne": "metadata"}, "component_type": "NATURAL-PROGRAM", "application": appname,
...
def expandedgetCode():
    ...
    #     start = timeit.default_timer()
    #     cursy = db.codebase.find_one(
    #         {"$and": [{'component_name': component_name.split(".")[0]+".MAP" }, {'component_type': "MAP"}]},
    #         {"_id": 0})
    #     end = timeit.default_timer()
    #     # cursy['completed_in']=end-start
    #     return jsonify(cursy)
    ...
    # return jsonify(cursy)
    component_name = request.args.get('component_name')
    component_type = request.args.get('component_type').replace("COBOL","NATURAL-PROGRAM")
    # print(component_name)
    # print(component_type)
    # if component_type!='scr':
    component_type = request.args.get('component_type')
    ...
    if component_type == "PDA" or component_type == "DDM" or component_type == "LDA":
        ...
        start = timeit.default_timer()
        cursy = db.codebase.find_one(
            {"$and": [{'component_name': component_name.split(".")[0] + ".NAT"}, {'component_type': component_type}]},
            {"_id": 0})
        end = timeit.default_timer()
        # cursy['completed_in']=end-start
        return jsonify(cursy)
    ...
    try:
        ...
        if search_filter:
            ...
            for k, v in my_dict.items():
                ...
                if temp_comp[1].upper() == "NAT":
                    temp_json['component_type'] = "NATURAL-PROGRAM"
...
def varImpact():
    ...
    try:
        ...
        if override_filter == 'yes':
            ...
            resultdf = dfDataset[dfDataset['sourcestatements'].to_frame().apply(lambda x: x.str.contains(search_filter, na=False)).any(axis=1)]
            ...
            if search_filter:
                ...
                if exact_match_filter == "yes" :
                    ...
                    resultdf = dfDataset[dfDataset['sourcestatements'].to_frame().apply(lambda x: x.str.contains(search_filter,case= True)).any(axis=1)]
                    ...
                    dfDataset = pd.DataFrame.from_records(cursor)
                    #print("dfDataset", dfDataset)
                    ##print("datatype",type(dfDataset['sourcestatements']), type(dfDataset))
                    resultdf = dfDataset[dfDataset['sourcestatements'].to_frame().apply(lambda x: x.str.contains(search_filter, case= False)).any(axis=1)]
                    # split_str=str(dfDataset['sourcestatements']).split("\n")
                    # for str_out in split_str:
                    #     if(str_out.find(search_filter)>=0):

                    ##print("resultdf3", resultdf, type(resultdf))
                    ##print(resultdf['sourcestatements'])
                    ##print('Result count', resultdf['component_name'].count())
                    data_object['displayed_count']=int(resultdf['component_name'].count())
                    data_object['data'] = resultdf.fillna("").to_dict('records')
                    ##print(data_object)
...
def usernameValidation():
    ...
    if request.method == 'POST':
        ...
        if username == "admin" and password == "admin@123":
            ...
            if len(session) > 0 and session['username'] ==username and session['password']  == password :
