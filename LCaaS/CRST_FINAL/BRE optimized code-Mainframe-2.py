...
client = MongoClient(config_full.database['hostname'], config_full.database['port'])
#db = client[config.database['database_name']]
db=client["CRST_FULLV1"]

cobol_folder_name = config_full.codebase_information['COBOL']['folder_name']
cobol_extension_type = config_full.codebase_information['COBOL']['extension']
COPYBOOK = config_full.codebase_information['COPYBOOK']['folder_name']

code_location =config_full.codebase_information['code_location']
ConditionPath=config_full.codebase_information['condition_path']
...
def main():
 ...
 wb1 = xlrd.open_workbook("D:\\CRST\\sample.xlsx")
 ...
 for filename in glob.glob(os.path.join(CobolPath,'*.cbl')):
    ...
    textfile = open("Copy_Expanded_Data.txt")
    ...
    for line in reversed(lines):
        #line = line[6:72]
        module = re.findall(r'^[A0-Z9].*[-]*.*[.]', line, re.IGNORECASE)
        if module != []:
    ...
    with open("another_file" + '.txt', "r+") as expanded_file:
        reversed_line = expanded_file.readlines()
        for line in reversed(reversed_line):
            #print(line)
                          ...
                          if onelinebuffer!=[]:
                            ...
                            if firstword in Key_Word_List and actualline.strip().endswith('.'):
                                ...
                                continue
                                    ...
                                    onelinebuffer.append(actualline)
                                    continue
                       elif actualline.strip().endswith('.'):
                           if onelineflag:
                                     # print(actualline)
                                      onelineflag = False
    ...
    with open("Duplicatefile0" + str(i) + '.txt',  'r') as PROC_DIV_CODE:
        ...
        for line_counter, line in enumerate(PROC_DIV_CODE):
            ...
            if line.strip() == "" or line.strip().startswith('*'):
                continue
            if not re.match('.*eject\s.*', line, re.IGNORECASE):
...
def file_operation(i,METADATA,filename):
    ...
    with open('output' + str(i) + '.txt', 'r+') as outfile:
     linenumber=0
     rule_number=0
     done=0
     perform_flag = False
