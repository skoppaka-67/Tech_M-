...
def getCode():
    ...
    if not re.match('\S+\.\S+', component_name.strip()):
        ...
        master_component_types = {'COBOL': 'cbl', 'COPYBOOK':'cpy','JCL':'jcl','PROC':'prc','BMS':'bms','SYSIN':'sysin','SCR':'scr','SCRIPTS':'scr'} #This line to be copied from the script
    ...
    if component_type == 'BMS':
        ...
        cursor = db.cics_field.find({"map_name":component_name.split(".")[0]})
        for map1 in cursor:
            print(map1)
            if map1["map_name"] == component_name.split(".")[0]:
               component_name = map1["mapset_name"] + ".bms"
               break
            break
    ...
    cursy = db.codebase.find_one({"$and":[{'component_name': component_name},{'component_type': component_type.upper()}]},{"_id":0})
...
def expandedgetCode():
    ...
    if not re.match('\S+\.\S+', component_name.strip()):
        ...
        master_component_types = {'COBOL': 'cbl', 'COPYBOOK': 'cpy', 'JCL': 'jcl', 'PROC': 'proc',
                                  'SYSIN': 'sysin'}  # This line to be copied from the script
        # Search for component extension via lookup and
        if component_type in master_component_types:
            component_name = component_name.strip() + '.' + master_component_types[component_type]

    start = timeit.default_timer()
    cursy = db.expanded_codebase.find_one({"$and": [{'component_name': component_name}, {'component_type': component_type}]},
                                 {"_id": 0})
    end = timeit.default_timer()
    cursy['completed_in'] = end - start
    return jsonify(cursy)
...
def cycComplex():
    cursor = db.master_inventory_report.aggregate([
        {"$group": {
            ...
            "count": {"$sum": 1},
            "total": {"$sum": "$cyclomatic_complexity"}
        }},
        {"$sort": {"total": -1}},
        {"$group": {
            "_id": "null",
    ...
    counter = 0
    for i in cursor:
        for app,cc in i.items():
            counter = counter + 1
            if counter == 6:
                break
            else:
                temp_dict = {}
                temp_dict["application_name"] = app.upper()
                temp_dict["cyclomatic_complexity"] = cc

                CC_list.append(temp_dict)
    # print("CC_list:", CC_list)
    OUTPUT_DATA["cyclomatic_complexity"] = CC_list
    return jsonify(OUTPUT_DATA)
...
#     #print('abcd',mylist)
#     #print(pb)
#     final["cyclomatic_complexity"]=mylist[:5]
#     #print('final', final)
#
#     #print('data',data)
#
...
def inbboundOutbound():
    ...
    # print('inbond', inbound_dict)
    sorted_d = {k: v for k, v in sorted(inbound_dict.items(), key=lambda item: item[0])}
    first10pairs_inbound = {k: sorted_d[k] for k in sorted(sorted_d.keys()) if (k != '')}
    ordered_dict_inbound = dict(sorted(first10pairs_inbound.items(), key=operator.itemgetter(1), reverse=True))
    countc = 0
    outbound_dict = {}
    for p in meta_data1:
    ...
    # print('outbound', outbound_dict)
    sorted_data = {key1: value1 for key1, value1 in sorted(outbound_dict.items(), key=lambda item: item[0])}
    first10pairs_outbound = {key1: sorted_data[key1] for key1 in sorted(sorted_data.keys()) if key1 != ''}
    ordered_dict_outbound = dict(sorted(first10pairs_outbound.items(), key=operator.itemgetter(1), reverse=True))
    whole_key = []
    ...
    in_descending = OrderedDict(sorted(dic.items(),
                                       key=lambda kv: kv[1], reverse=True)[:5])
    out_descending = OrderedDict(sorted(dic1.items(),
                                        key=lambda kv: kv[1], reverse=True))

    # print(in_descending)
    # print(out_descending)

    sort_in_dict ={}

    for in_key,in_value in in_descending.items():
        for ou_key,ou_value in out_descending.items():
            if in_key == ou_key:
                add_value = ou_value
                sort_in_dict[ou_key] = add_value

    # sort_in = OrderedDict(sorted(sort_in_dict.items(),key=lambda kv: kv[1], reverse=True))

    # in_data.append(dic)
    # out_data.append(dic1)
    in_data.append(in_descending)
    out_data.append(sort_in_dict)

    # in_data = [x for x in in_data1[0:5]]
    # print("Inbound:",in_data)
    # out_data = [x for x in out_data1[0:5]]
    # print("Outbound:",out_data)

    return jsonify({"inboundOutbound": {"inbound": out_data,
                                        "outbound": in_data}})  # Changed inbound to outbound and outbound to inbound
...
#     ordered_dict_inbound = dict(sorted(first10pairs_inbound.items(), key=operator.itemgetter(1), reverse=True))
#     # first10pairs_inbound = {k: ordered_dict_inbound[k] for k in sorted(ordered_dict_inbound.keys()[:5])if (k != '')}
#     # print("Ordered_dict:",first10pairs_inbound)
#     countc=0
#     outbound_dict = {}
#     for p in meta_data1:
#         outbound_list = []
...
#     out_data.append(dic1)
#
#     return jsonify({"inboundOutbound": {"outbound": in_data, "inbound": out_data}})#Changed inbound to outbound and outbound to inbound
...
#                     # Pandas code for cursor to dictionary convertion
#                     dfDataset = pd.DataFrame.from_records(cursor)
#                     data_object['displayed_count'] = db.cross_reference_report.find({"calling_app_name": appname}).count()
#                     data_object['data'] = dfDataset.fillna("").to_dict('records')
#                     return jsonify(data_object)
#
#
...
#                     search_filter = " " + search_filter + " "
#                     resultdf = dfDataset[
#                         dfDataset.apply(lambda x: x.str.contains(search_filter, case=True)).any(axis=1)]
#                     # print('Result count', resultdf['component_name'].count())
#                     data_object['displayed_count'] = int(resultdf['component_name'].count())
#                     data_object['data'] = resultdf.fillna("").to_dict('records')
#                     return jsonify(data_object)
...
#                     dfDataset = pd.DataFrame.from_records(cursor)
#                     resultdf = dfDataset[
#                         dfDataset.apply(lambda x: x.str.contains(search_filter, case=False)).any(axis=1)]
#                     # print('Result count', resultdf['component_name'].count())
#                     data_object['displayed_count'] = int(resultdf['component_name'].count())
#                     data_object['data'] = resultdf.fillna("").to_dict('records')
#                     return jsonify(data_object)
...
#                 #If there is no filter criteria, limit to 1000 records
#                 # Query all cross reference data
#                 cursor = db.varimpactcodebase.find({'type': {"$ne": "metadata"}}, {'_id': 0}).limit(CROSS_REFERENCE_LIMIT)
#                 # Query to fetch header details
#                 metadata = db.varimpactcodebase.find_one({"type": "metadata"}, {'_id': 0})
#                 data_object['headers'] = metadata['headers']
#
            ...
            for program in program_list:
                ...
                for pgm_name  in payload['data'][program]:
                    ...
                    if pgm_name['name'] == "External_Program":
                        ...
                        link_set.add(json.dumps({"source":'p_'+from_node,"target":'p_'+to_node,"label": 'External_Program'}))
    ...
    try:
            ...
            for i in connections:
                ...
                if  i["called_type"].lower() !="utility":
                    ...
                    response["nodes"].append({"id":"p_"+i["component_name"]+"(" + i["component_type"] + ")","label": i["component_name"]+"(" + i["component_type"] + ")"  })
                    response["nodes"].append({"id": "p_" + i["called_name"]+ "(" + i["called_type"] + ")", "label": i["called_name"] + "(" + i["called_type"] + ")"})
...
#                     for node in nodes:
#                         response['nodes'].append({"id": "p_" + node, "label": node})
#                 else:
...
@app.route('/api/v1/augment/masterInventory1', methods=['POST'])
def augmentMasterInventory1():
    try:
        # check if the text is json
        JsonVerified, verification_payload = jsonVerify(request.data)
        if JsonVerified:
        ...
        for ite in keys:
            row = payload[ite]
            print(ite)
            if db.master_inventory_report1.update_one({"component_name":ite+".cbl"}, {
                "$set":{"dead_para_count": row['dead_para_count'], "dead_para_list": row['dead_para_list'], "no_of_dead_lines": row['total_dead_lines'], 'total_para_count': row['total_para_count']}}).acknowledged:
                print('updated',ite)
            else:
                print('not updated')
...
def callchain():
    ...
    def child_link(component_name, called_name, component_type, called_type):
        if(component_name!=called_name or component_type!=called_type):
            response['links'].append({"source": "p_" + component_name+"["+component_type+"]",
                                      "target": "p_" + called_name+"["+called_type+"]",
                                      "label": "C"})
    ...
    def parent_link(called_name, component_type):
        response['links'].append({"source": "p_" + called_name,
                                  ...
                                  "label": "P"})
        return response['links']
    # def parent_link(component_name, called_name, component_type, called_type):
    #   # if (component_name != called_name or component_type != called_type):
    #     print("inside parent")
    #     response['links'].append({"source": "p_" +called_name+"["+called_type+"]" ,
    #                               "target": "p_" + component_name+"["+component_type+"]",
    #                               "label": "P"})
    #     return response['links']
    ...
    try:
        for c_name, c_type in zip(component_name, component_type):
            ...
            if parent_child=="child":
              name_type_map = {input_name+"["+input_type+"]": input_type}
              nodes.add(input_name + "[" + input_type + "]")
            else:
                        ...
                        if connection['called_name'].strip() != '':
                            ...
                            if connection['component_name'].strip() == input_name:
                                ...
                                if parent_child == "child":
                                    ...
                                    if connection['called_type'].lower() == "cobol"\
                                            or connection['called_type'].lower() == "proc":
                                        # print("type cobol or proc",connection['called_type'].lower() )
                                        # print("called name",connection['called_name'])
                                        # print("called type",connection['called_type'])
                                        # if connection['called_name'] not in component_name or\
                                        #         connection['called_type'] not in component_type :
                                        if connection['called_name'] != input_name or\
                                                connection['called_type']!=input_type:
                                        ...
                                        child_output = child_link(connection['component_name'],
                                                                  connection['called_name'],input_type,connection['called_type'] )
                                        name_type_map[connection['called_name']+"["+connection['called_type']+"]"] = connection['called_type']
                                        if connection['called_name']+"["+connection['called_type']+"]" not in nodes:
                                            print(nodes)
                                            nodes.add(connection['called_name']+"["+connection['called_type']+"]")
                                            # print(nodes)
                                ...
                                #   if connection['called_type'].lower() == "cobol" or connection['called_type'].lower() == "proc":
                                #     if connection['called_name'] != input_name or \
                                #       connection['called_type'] != input_type:
                                #         # parent_output = parent_link(connection['called_name'], connection['component_name'],connection['called_type'],input_type)
                                #         # print(parent_output)
                                #         # if connection['called_name'] != input_name or \
                                #         #   connection['called_type'] != input_type:
                                ...
                                #
                                #     parent_output = parent_link(connection['called_name'],
                                #                                   connection['component_name'],
                                #                                   connection['called_type'],
                                #                                   input_type)
                                #       # print("parent")
                                #       # print(component_name)
                                #     print("paernt output",parent_output)
                                #     name_type_map[connection['component_name']+"["+connection['component_type']+"]"] = connection['component_type']
                                #     if connection['component_name']+"["+connection['component_type']+"]" not in nodes:
                                #         nodes.add(connection['component_name']+"["+connection['component_type']+"]")
                                #         flag = 0
                                  if (parent_child == "parent"):
...
def glossary():
    ...
    try:
        ...
        if override_filter == 'yes':
            ...
            # #print('Result count', resultdf['component_name'].count())
            data_object['displayed_count'] = db.glossary.find({"component_name": option}).count()
            data_object['data'] = resultdf.fillna("").to_dict('records')
            # dfDataset = pd.DataFrame.from_records(cursor)
            # resultdf = dfDataset[dfDataset.apply(lambda x: x.str.contains(search_filter, na=False)).any(axis=1)]
            # data_object['data'] = resultdf.fillna("").to_dict('records')
...
def uploadglossary():
        ...
        # str_file.append(eval(file.decode("utf-8")))
        ...
        # option = str_file[0]['component_name']
...
def updateVarDef():
    ...
    component_name = request.args.get('component_name')
    # time.sleep(2)
    def_update.main(app_name, component_name)
...
def cicsxrefApp():
    ...
    try:
        ...
        if appname == "All":
            # Fetch the total record count
            # data_object['total_record_count'] = db.cross_reference_report.count_documents({}) - 1
            data_object['total_record_count'] = db.cross_reference_report.find({"$or": [{"$and": [{"disp": {"$gt": ""}}, {
                    "$or": [{"component_type": "COBOL"}, {"component_type": "COPYBOOK"}]}]}, {
                                                                     "$and": [{"disp": {"$gt": ""}}, {
            ...
            if override_filter == 'yes':
                ...
                cursor = db.cross_reference_report.find({"$or": [{"$and": [{"disp": {"$gt": ""}}, {
                    "$or": [{"component_type": "COBOL"}, {"component_type": "COPYBOOK"}]}]}, {
                                                                     "$and": [{"disp": {"$gt": ""}}, {
                                                                         "$or": [{"component_type": "COBOL"},
                                                                                 {"component_type": "COPYBOOK"}]}]}]},
                                                         {'_id': 0}).limit(CROSS_REFERENCE_LIMIT)
                ...
                if search_filter:
                    # Query all cross reference data
                    # cursor = db.cicsxref.find({'type': {"$ne": "metadata"}}, {'_id': 0})
                    cursor = db.cross_reference_report.find( {"$or" : [ { "$and" : [ {"disp" :{"$gt":""}}, { "$or":[ { "component_type": "COBOL" },{ "component_type": "COPYBOOK" }]}]},  {"$and" : [ {"disp" :{"$gt":""}}, {"$or":[{ "component_type": "COBOL" },{ "component_type": "COPYBOOK" } ]}]}]},{'_id': 0} )
...
{"$or" :
   [
    ...
    { "$and" :
        ...
        [ {"disp" :{"$gt":""}} ,{"calling_app_name":appname},
            {"$or":[ { "component_type": "COBOL" },{ "component_type": "COPYBOOK" }]}]},
            {"$and" : [ {"disp" :{"$gt":""}},{"calling_app_name":appname},
            {"$or":[{ "component_type": "COBOL" },{ "component_type": "COPYBOOK" } ]}]}]},{'_id': 0})
   ...
   [
    ...
    { "$and" :
        ...
        [ {"disp" :{"$gt":""}} ,{"calling_app_name":appname},
            {"$or":[ { "component_type": "COBOL" },{ "component_type": "COPYBOOK" }]}]},
            {"$and" : [ {"disp" :{"$gt":""}},{"calling_app_name":appname},
            {"$or":[{ "component_type": "COBOL" },{ "component_type": "COPYBOOK" } ]}]}]},{'_id': 0}).limit(CROSS_REFERENCE_LIMIT)
...
#             # Fetch the total record count
#             # data_object['total_record_count'] = db.cross_reference_report.count_documents({}) - 1
#             data_object['total_record_count'] = db.cross_reference_report.find({"$or": [{"$and": [{"disp": {"$ne": ""}}, {
#                     "$or": [{"component_type": "COBOL"}, {"component_type": "COPYBOOK"}]}]}, {
#                                                                      "$and": [{"dd_name": {"$ne": ""}}, {
#
#                                                                          "$or": [{"component_type": "COBOL"},
...
#                 # Query all cross reference data
#                 # cursor = db.cicsxref.find({'type': {"$ne": "metadata"}}, {'_id': 0})
#                 cursor = db.cross_reference_report.find({"$or": [{"$and": [{"disp": {"$ne": ""}}, {
#                     "$or": [{"component_type": "COBOL"}, {"component_type": "COPYBOOK"}]}]}, {
#                                                                      "$and": [{"dd_name": {"$ne": ""}}, {
#
#
...
#                     # Query all cross reference data
#                     # cursor = db.cicsxref.find({'type': {"$ne": "metadata"}}, {'_id': 0})
#                     cursor = db.cross_reference_report.find( {"$or" : [ { "$and" : [ {"disp" :{"$ne":""}}, { "$or":[ { "component_type": "COBOL" },{ "component_type": "COPYBOOK" }]}]},  {"$and" : [ {"dd_name" :{"$ne":""}}, {"$or":[{ "component_type": "COBOL" },{ "component_type": "COPYBOOK" } ]}]}]},{'_id': 0} )
#
#                     # Query to fetch header details
#                     metadata = db.cross_reference_report.find_one({"type": "metadata"}, {'_id': 0})
#                     data_object['headers'] = metadata['headers']
...
#     { "$and" :
#
#         [ {"disp" :{"$ne":""}} ,{"calling_app_name":appname},
#             {"$or":[ { "component_type": "COBOL" },{ "component_type": "COPYBOOK" }]}]},
#             {"$and" : [ {"dd_name" :{"$ne":""}},{"calling_app_name":appname},
#             {"$or":[{ "component_type": "COBOL" },{ "component_type": "COPYBOOK" } ]}]}]},{'_id': 0})
#
#
#
...
def inout():
  ...
  # connections = db.cross_reference_report.find({"type": {'$ne': 'metadata'}})
  connections = db.cross_reference_report.find({"$and":[{"type": {'$ne': 'metadata'}},{"called_type":{'$ne': 'FILE'}},{"calling_app_name":{'$ne': 'Common Utility Modules'}},{"calling_app_name":{'$ne': 'New Prefix. Yet to get mapping'}},{"called_app_name":{'$ne': 'New Prefix. Yet to get mapping'}},{"called_app_name":{'$ne': 'Common Utility Modules'}},{"called_type":{'$ne': 'Utility'}}]})
  calling_list = []
  called_list = []
  d = {}
  metadata = []
  for connection in connections:
    ...
    try:
      # if connection['calling_app_name'] == "Unknown" or connection['called_app_name'] == "Unkown":
      #     continue
      calling_list.append(connection['calling_app_name']) #call_app
      called_list.append(connection['calling_app_name'] + connection['called_app_name'])#call_app+called
  ...
  for k, v in di.items():
    for li in list(set(v)):
      if k != li:
        ...
        # print(k,li,count)
        if inout== "inbound":
          out_val.append([k+"["+str(count)+"]", li, count])
        elif inout=="outbound":
          out_val.append([k , li+"[" + str(count)+"]", count])
...
def spiderFilterDetails():
  ...
  try:
    ...
    name_type_map = {response['component_name']+" ("+response['component_type']+")": response['component_type']}
    # Adding the first node of the chart
    nodes = {response['component_name']+" ("+response['component_type']+")"}
    print("1", nodes)
    # print(response['component_name'])
    if response['component_name'] is not None:
      try:
          ...
          for connection in connections:
            ...
            if connection['called_name'].strip() != '':
              ...
              if connection['component_name'] == response['component_name']:
                if (str(connection['called_type']).upper() == str(response['option']).upper()):
                  if (connection['component_name'] != connection['called_name']):
                    response['links'].append({"source": "p_" + connection['component_name']+" ("+connection['component_type']+")",
                                              "target": "p_" + connection['called_name']+" ("+connection['called_type']+")",
                                              "label": "C"})
                  name_type_map[connection['called_name']+" ("+connection['called_type']+")"] = connection['called_type']
                  nodes.add(connection['called_name']+" ("+connection['called_type']+")")
                  nodes.add(connection['component_name']+" ("+connection['component_type']+")")
                if (response['option'] == "All"):
                  if (connection['component_name'] != connection['called_name'] or connection['component_type'] != connection['called_type']):
                    print("insdie link", connection['component_name'], connection['component_type'],connection['called_name'], connection['called_type'])
                    response['links'].append({"source": "p_" + connection['component_name']+" ("+connection['component_type']+")",
                                              "target": "p_" + connection['called_name']+" ("+connection['called_type']+")",
                                              "label": "C"})
                  name_type_map[connection['called_name']+" ("+connection['component_type']+")"] = connection['called_type']
                  nodes.add(connection['called_name']+" ("+connection['called_type']+")")
                  nodes.add(connection['component_name']+" ("+connection['component_type']+")")
              # If the CALLED component's name is same as option
              else:
                if (str(connection['component_type']).upper() == str(response['option'])):
                  if (connection['component_name'] != connection['called_name'] or
                  connection['component_type'] != connection['called_type']):
                    response['links'].append({"source": "p_" + connection['called_name']+" ("+connection['called_type']+")",
                                              "target": "p_" + connection['component_name']+" ("+connection['component_type']+")",
                                              "label": "P"})
                  name_type_map[connection['component_name']+" ("+connection['component_type']+")"] = connection['component_type']
...
def comment_line_report():
    ...
    # print(cursors)
    if apname == "All":
        cursors = db.cobol_output.find({'type': {"$ne": "metadata"}}, {'_id': 0})
        for cursor in cursors:
            json_format = {"component_name": cursor["component_name"], "component_type": cursor["component_type"],
...
#                 {"type": {"$ne": "metadata"}, "component_name" : { "$nin" : ["TPL004C.cbl","TPL015C.cbl","TPL030C.cbl","CIC762.cbl","DB2DUMMY.cbl","OTC888.cbl","SRS798B.cbl","CAS120.cbl","CCP03OIL.cbl","CCP60OOI.cbl","ACR015.cbl","BRG240O.cbl","BRG250O.cbl","BRG230O.cbl", "ACR031.cbl","ACR91X.cbl","AVR001B.cbl","AVR004B.cbl","BRG220O.cbl"] },
# "component_type": "COBOL", "no_of_dead_lines": {"$gt": 1}},
...
def deadparalist():
    ...
    if appname == "All":
        try:
            ...
            cursor = db.master_inventory_report.find(
                {"type": {"$ne": "metadata"}, "component_name" : { "$nin" : ["TPL004C.cbl","TPL015C.cbl","TPL030C.cbl","CIC762.cbl","DB2DUMMY.cbl","OTC888.cbl","SRS798B.cbl","CAS120.cbl","CCP03OIL.cbl","CCP60OOI.cbl","ACR015.cbl","BRG240O.cbl","BRG250O.cbl","BRG230O.cbl", "ACR031.cbl","ACR91X.cbl","AVR001B.cbl","AVR004B.cbl","BRG220O.cbl"] },
 "component_type": "COBOL", "dead_para_count": {"$gt": 0}},
                {"component_name": 1, "component_type": 1, "application": appname, "dead_para_list": 1,
                 "dead_para_count": 1, "no_of_dead_lines": 1, "_id": 0})
            metadata = db.master_inventory_report.find_one({"type": "metadata"}, {"_id": 0})
            datavalue["headers"] = headers
...
def bre_2():
    ...
    try:
        if option == "":
            ...
            option = option[0]
            # print(option)
            rec_count = db.bre_report2.find({"pgm_name": option}).count()
            # cursor = db.bre_report2.find({"pgm_name": option}, {"_id": 0}).sort('_id')
            override_filter = request.args.get("overrideFilter")
            # print("REC:",rec_count)
            if override_filter == 'no':

                if rec_count <= 5000:
                    cursor = db.bre_report2.find({"pgm_name": option}, {"_id": 0}).sort('_id')
                    datavalue["msg"] = ""
                else:
                    cursor = db.bre_report2.find({"pgm_name": option}, {"_id": 0}).sort('_id').limit(5000)
                    datavalue["msg"] = "Report has more records. Please download and view the report."
            elif override_filter == 'yes':
                     cursor = db.bre_report2.find({"pgm_name": option}, {"_id": 0}).sort('_id')
                     datavalue["msg"] = ""
            datavalue["headers"] = header
            for record in cursor:
...
def bre():
    ...
    try:
        if option == "":
            ...
            option = option.split('.')
            option = option[0]
            # rec_count = db.bre_rules_report.find({"pgm_name": option + "."}).count()
            # override_filter = request.args.get("overrideFilter")
            # print("REC:",rec_count)
            # if override_filter == 'no':
            #     if rec_count <= 500:
            ...
            #         datavalue["msg"] = ""
            #     else:
            #         cursor = db.bre_rules_report.find({"pgm_name": option+"."}, {"_id": 0}).sort('_id').limit(500)
            #         datavalue["msg"] = "Report has more records. Please download and view the report."
            #         # print("Else:",rec_count)
            # elif override_filter == 'yes':
            cursor = db.bre_rules_report.find({"pgm_name": option + "."}, {"_id": 0}).sort('_id')
            datavalue["msg"] = ""
            datavalue["headers"] = header
...
def chartsAPI():
    ...
    try:
        ...
        for ite in cursor:
            OUTPUT_DATA['largest_applications'][ite['_id']['application']] = ite['count']
        # OUTPUT_DATA['largest_applications'] = OUTPUT_DATA['largest_applications'][:10]
        ####################################################
        # Fetching all the applications and their DEAD LOC #
        ####################################################
        ...
        for i in cursor:
            for app, dead_lines in i.items():
                ...
                if counter == 6:
                    ...
                    temp_dict = {}
                    temp_dict["application_name"] = app.upper()
                    temp_dict["no_of_dead_lines"] = dead_lines

                    dead_code_list.append(temp_dict)
        # print("CC_list:", dead_code_list)
        OUTPUT_DATA["largest_applications_dead_code"] = dead_code_list
        ...
        COMPONENT_TYPES = db.master_inventory_report.distinct("component_type")
        # COMPONENT_TYPES = ['COBOL','APS','JCL','COPYBOOK','PROC','SYSIN','BMS']
        # #print('Tech Components', COMPONENT_TYPES)
        for TYPE in COMPONENT_TYPES:
            loc = 0
            try:
                # cursor = db.master_inventory_report.aggregate(
                #    [{"$match": {"componentype": TYPE}}, {"$group": {"_id": "null", "count": {"$sum": "$Loc"}}}])
                # for row in cursor:
                #    loc = row['count']
                #    #print(TYPE,loc)
                # OUTPUT_DATA['techstack_vs_loc'][TYPE] = loc
        ...
        # Processing application vs tech stack vs LOC
        time_app_tech_loc_start = timeit.default_timer()
        COMPONENT_TYPES1 = ['COBOL','APS','JCL']
        for TYPE in COMPONENT_TYPES1:
            OUTPUT_DATA['app_tech_loc'][TYPE] = {}
            applist = db.master_inventory_report.distinct("application")
            #comp_typ_list = ['JCL','COPYBOOK','COBOL']
            comp_typ_list = db.master_inventory_report.distinct("component_type")
            appdict = {}
            d_ascending = {}
            cop_ascending = {}
            res_dict = {}
            ...
            out = {}
            type_app_dict = {}
            for type in comp_typ_list:
                for app in applist:
                    loc_total = 0
                    cursy = db.master_inventory_report.find(
                        {'$and': [{"application": app}, {"component_type": type}, {'type': {'$ne': "metadata"}}]})
                    for rec in cursy:
                        loc_total = loc_total + rec['Loc']
                    appdict[app] = loc_total
                    # print("APP:",appdict)
                    if type == "COBOL":
                        d_ascending = OrderedDict(sorted(appdict.items(), key=lambda kv: kv[1], reverse=True))
                        # print("Cobol:",d_ascending)
                        out = dict(list(d_ascending.items())[0:10])
                        # print("D_out:",out)
                    else:
                        cop_ascending = OrderedDict(sorted(appdict.items(), key=lambda kv: kv[1], reverse=True))
                        # print("COpybook:",d_ascending)
                ...
                for out_key,out_value in out.items():
                    for cop_key,cop_value in cop_ascending.items():
                        if out_key == cop_key:
                            print("Result:",cop_key,cop_value)
                            res_dict[out_key] = cop_value

                if type == "COBOL":


                    type_app_dict[type] = out
                    # print("Cobol_Type:",type_app_dict)
                    OUTPUT_DATA['app_tech_loc'] = type_app_dict
                else:
                    type_app_dict[type] = res_dict
                    # print("Copybook_Type:",type_app_dict)
                    OUTPUT_DATA['app_tech_loc'] = type_app_dict
            # print(json.dumps(type_app_dict,indent=4))

            # print(json.dumps(OUTPUT_DATA, indent=4))
            # for app in app_list[:5]:
            #     OUTPUT_DATA['app_tech_loc'][TYPE][app] = 0
            # for APP in APPLICATIONS_MASTER_LIST:
            #     loc = 0
            #     try:
            #         cursor = db.master_inventory_report.aggregate(
            ...
            #             OUTPUT_DATA['app_tech_loc'][TYPE][APP] = loc
            #
            #         print("Check:",OUTPUT_DATA['app_tech_loc'])
            #         # OUTPUT_DATA['app_tech_loc']
            #     except Exception as e:
            #         print('Error while fetching Tech stack vs LOC:  ' + str(e))
            #         # print(traceback.format_exc())
        time_app_tech_loc_stop = timeit.default_timer()
...
def rulesChartAPI():
    ...
    for var in a:
        for key, value in var.items():
            # print("key:",key,value)
            if any(x.isalpha() for x in key) and any(x.isspace() for x in key):
                new_dict[key] = value
...
def buisnessConnected():
    ...
    N=5
    c = (dict(ChainMap(*b)))
    cd = dict(sorted(c.items(), key=operator.itemgetter(1), reverse=True))
    out = dict(itertools.islice(cd.items(), N))
    final_data = out
    # print(final_data)
