...
start= ('Public Function','Private Function','Function','Public Sub','Private Sub','Sub')
vb = config.codebase_information['VB']['folder_name']
code_location = config.vb6_filespath
vb_path = code_location
# vb_path = code_location + '\\*'
...
class Database:
    ...
    def __init__(self):
        ...
        self.db_name = config.vb6_dbname
        self.col_name = config.flowchart
...
class PreProcessing():
    ...
    def get_files(self):
        ...
        for root, dirs, files in os.walk(vb_path):
            for file in files:
                if file.lower().endswith(tuple([item.lower() for item in [".frm", ".bas", ".dsr"]])):
    ...
    def remove_comment(self,line):
        ...
        if any(["'" in item for item in line.split('"')[0::2]]):
            output = min([line[:index] for index in indexes if line[:index].count('"') % 2 == 0], key=len)
        else:
            output = line
        return output
    ...
    def tagging_end_if_to_singleline_id(self, file):
        ...
        for append_lines in input_file.readlines():
            ...
            if if_collecting_flag:
                # print('ifff',append_lines,main_if_index)
                if append_lines[main_if_index] != " " and not re.search('.* end if.*', append_lines,
                    ...
                    # print('inside',append_lines, append_lines[8], main_if_index)
    ...
    def break_code_into_blocks(self, processed_line_list):
        ...
        for line in processed_line_list:
            try:
                if (line.lower().__contains__("end sub") or line.lower().__contains__("end function")):
                    # print('subb',line,function_name)
                    fun_flag = False
                    code_block_hash[function_name] = copy.deepcopy(code_block_list)
                    # print('codeblock',code_block_hash)
                    function_name = ''
                ...
                #     "private")) or line.lower().__contains__("protected") or line.lower().startswith("function"):
                if line.strip().startswith(start):
                    ...
                    if line.strip().startswith("Public Shared Function"):
                        ...
                        line1 = line.replace(" Sub", "Function").replace(" sub", "Function").replace("function",
                                                                                                     "Function")\
                            .replace("Sub", "Function").replace("sub", "Function").replace("function",
                                                                                                     "Function")

                        # function_name = line1.split("Function")[1].split("(")[0]
                        function_name = line.split('(')[0].split()[-1]
                        # print('nameee', function_name)
                        continue
                if fun_flag:
                    # print(line)
        ...
        # print(json.dumps(code_block_hash, indent=4))
...
class flowchartProcessing(PreProcessing, Database):
    def __init__(self, DB_CON, pre_prop):
        ...
        # cursy = DB_CON.Db_conn(db, "Pub_Fun_Lookup")
    ...
    def block_separation(self, code_block_dict, file_name):
        ...
        # Insert into DB
        try:
            DB_CON.db_update("VB6_TEST", "para_flowchart_data", OUTPUT_DATA)
            OUTPUT_DATA.clear()
            print('update sucess')
        except Exception as e:
            print('Error:' + str(e))
