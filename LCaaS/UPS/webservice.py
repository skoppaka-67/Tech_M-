...
def deadparalist():
   ...
   if appname =="All":
       try:
        headers=["component_name","component_type","application","dead_para_list","dead_para_count","no_of_dead_lines"]
        cursor = db.master_inventory_report.find({"type":{"$ne":"metadata"},"component_type":"COBOL","dead_para_count":{"$gt":0}},{"component_name":1,"component_type":1,"application": appname,"dead_para_list":1,"dead_para_count":1,"no_of_dead_lines":1,"_id":0})
       ...
       return jsonify(datavalue)
           ...
           try:
               ...
               cursor = db.master_inventory_report.find(
                   ...
                   {"component_name": 1, "component_type": 1,"application": appname, "dead_para_list": 1, "dead_para_count": 1,
...
def procedureFlowCompList():
    ...
    if option is not None:
        ...
        try:
            response = db.master_inventory_report.find(
                {"application": application_name , "component_type": "COBOL"})
...
def bre():
    ...
    try:
        ...
        option=option.split('.')[0] + ".cbl"
        print("BRE:",option)
        if option != "":
            header = ["fragment_Id", "para_name", "source_statements", "rule_category", "statement_group", "parent_rule_id"]
            datavalue["headers"] = header
            cursor = db.bre_rules_report.find({"pgm_name": option+".cbl"}, {"_id": 0}).sort('_id')
            for record in cursor:
                datavalue["data"].append(record)

        if option.strip()!="":
            if option.__contains__('.CO'):
                None
            else:
             option=option
            print(option)
            application=db.master_inventory_report.find_one({"component_name":option})
            #application=application["application"]
            #collection_name="bre_rules_report_"+application.split()[0].lower()
            print("here")
            header = ["fragment_Id", "para_name", "source_statements", "rule_category", "statement_group", "parent_rule_id"]
            #option = option.split('.')
            #option = option[0]
            #cursor = db.bre_rules_report.find({"pgm_name": option}, {"_id": 0}).sort('_id')
            if application=="Accounts Payable":
                cursor=db.bre_rules_report_ap.find({"pgm_name": option}, {"_id": 0}).sort('_id')
            elif application=="Accounts Receivable":
                cursor = db.bre_rules_report_ar.find({"pgm_name": option}, {"_id": 0}).sort('_id')
            else:
                cursor =db.bre_rules_report.find({"pgm_name": option+"cbl"}, {"_id": 0}).sort('_id')
                # print(cursor)
            datavalue["headers"] = header
            ...
            header = ["fragment_Id", "para_name", "source_statements", "rule_category", "statement_group",
                      "parent_rule_id"]
            datavalue["headers"] = header
            datavalue["data"]=[]
    except :
...
def getCode():
    ...
    if not re.match('\S+\.\S+', component_name.strip()):
        ...
        master_component_types = {'COBOL': 'CO', 'COPYBOOK': 'cpy', 'JCL': 'jcl', 'PROC': 'proc', 'SYSIN': 'sysin',
...
def expandedgetCode():
    ...
    if not re.match('\S+\.\S+', component_name.strip()):
        # Component does not have extension,
        master_component_types = {'COBOL': 'CO', 'COPYBOOK': 'cpy', 'JCL': 'jcl', 'PROC': 'proc',
...
#
# {
#   "businessConnectedRules": {
#     "Business Rule": {
#       "Inventory Management ": 144279,
#       "Common modules": 75125
#     }
#   }
# }
...
def buisnessConnected():
    ...
    for key1 in applist:
        if key1 == None:
            ...
            cbr_count = 0
            # for each in val:
            query = db.bre_rules_report.count_documents({'$and': [{"application": key1}, {"rule_category": "Business Rule"}]})
            query1 = db.bre_rules_report.count_documents(
                {'$and': [{"application": key1}, {"rule_category": "Connected Business Rule"}]})
...
#             continue
#         else:
#             for comp in master_data:
#                 if comp["application"] == app:
#                     list1.append(comp["component_name"].split(".")[0])
#             app_pgm[app] = copy.deepcopy(list1)
#
...
#         cbr_count = 0
#         for each in val:
#             query = db.bre_rules_report.count_documents(
#                 {'$and': [{"pgm_name": each}, {"rule_category": "Business Rule"}]})
#             query1 = db.bre_rules_report.count_documents(
#                 {'$and': [{"pgm_name": each}, {"rule_category": "Connected Business Rule"}]})
#             br_count = br_count + query
#             cbr_count = cbr_count + query1
#         # print('final_key',count,key1)
#         buisiness[key1] = br_count
...
############
    ...
    #             {'$and': [{"pgm_name": each}, {"rule_category": "Connected Business Rule"}]}).count()
    #         br_count = br_count + query
    #         cbr_count = cbr_count + query1
    #     # print('final_key',count,key1)
    #     buisiness[key1] = br_count
...
def masterInventory():
    ...
    if appname == "All":
        ...
        try:
            cursor = db.master_inventory_report.find({},{'_id': 0})
            metadata = db.master_inventory_report.find_one({"type": "metadata"}, {'_id': 0})
            data_object['headers'] = metadata['headers']
            for document in cursor:
                data_object['data'].append(document)
...
def CRUD():
    ...
    try:
        if appname == "All":
            ...
            cursor = db.crud_report.find({}, {'_id': 0})
            metadata = db.crud_report.find_one({"type": "metadata"}, {'_id': 0})
            data_object['headers'] = metadata['headers']
            for document in cursor:
                data_object['data'].append(document)
...
def controlflow():
    ...
    try:
        ...
        for i in table_connections:
            print(i["component_name"],response['component_name'])
            if i["component_name"] == response['component_name']:  # inputcomponent:
                response["nodes"].append({"id": "p_" + i["File"], "label": i["File"] + "(" + "File" + ")"})
...
def augmentMasterInventory():
    ...
    # <
    #
    # class 'dict'>: {'dead_para_count': 0, 'dead_para_list': [], 'total_dead_lines': 0, 'total_para_count': 6}
    ...
    try:
        ...
        if 'data' in keys:
            ...
            if len(db_data['data']) == 0:
                ...
                # Delete all existing rows (Nuking all data f                previousDeleted = False
                try:
...
def callchain():
    ...
    # component_name1 = component_name
    if component_name1.__contains__("HASH"):  # HOT FIX FOR & response
        component_name1 = component_name1.replace("HASH", "#")
...
# @app.route("/api/v1/masterInventory")
# def masterInventory():
#     data = []
#     appname  = request.args.get("option")
#     connections = db.master_inventory_report.find({"application" : appname})
#     for connection in connections:
