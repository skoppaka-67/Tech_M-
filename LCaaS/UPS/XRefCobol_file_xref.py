                        ...
                        if line[0] == '*' or line[0]=='#':
                            ...
                            if re.match('^\s*CALL\s.*', line,re.IGNORECASE) or re.match('^\s*call .*', line) :
                                ...
                                if re.match('\'.*\'', line.split()[1]) or re.match('".*"', line.split()[1]):
                                    ...
                                    METADATA.append({'component_name': COMPONENT_NAME.split('.')[0], 'component_type': 'COBOL',
                                                     'called_name': line.split()[1][1:-1].replace('"',"").replace('.',''), 'called_type': 'COBOL',
                                    ...
                                    if re.match('\'.*\'', line.split()[2]) or re.match('".*"', line.split()[2]):
                                        ...
                                        METADATA.append(
                                            {'component_name': COMPONENT_NAME.split('.')[0], 'component_type': 'COBOL',
                                             'called_name': line.split()[2][1:-1].replace('"', "").replace('.', '')+'.CO',
                                             'called_type': 'COBOL',
                                             'calling_app_name': 'UNKNOWN', 'called_app_name': 'UNKNOWN',
                                             'step_name': '', 'disp': '', 'comments': '', 'dd_name': '',
                                             'file_name': ''})
                                ...
                                if re.match(re.compile('.*\s' + var + '\s.*'), line):
                                    if re.match(re.compile('.*\s' + var + '\s.*\.'), line):
                                        ...
                                        if re.match('.*\sVALUE\s.*', line) or re.match('.*value.*', line):
                                            ...
                                            if ((line.split()[len(line.split()) - 1])== "\'." )or((line.split()[len(line.split()) - 1]) == '\".'):
                                                #print("extra", line)
                                                extracted_constant = \
                    ...
                    try:
                        ...
                        if line[0] == '*':
                            ...
                            if re.match('^\s*CALL\s.*', line) or re.match('^\s*call.*', line) :
                                ...
                                if re.match('\'.*\'', line.split()[1]) or re.match('".*"', line.split()[1]):
                                    # Direct call within quotes found, hence adding to metadata directly
                                    METADATA.append({'component_name': COMPONENT_NAME.split('.')[0], 'component_type': 'COPYBOOK',
                                                     'called_name': line.split()[1][1:-1].replace('"',"").replace('.',''), 'called_type': 'COBOL',
                                                     'calling_app_name': 'UNKNOWN', 'called_app_name': 'UNKNOWN',
                                                     'step_name': '', 'disp': '', 'comments': '', 'dd_name': '',
                                                     'file_name': ''})
    ...
    for eachFile in COBOL_FILE_LIST1:
        ...
        if exists:
            ...
            with open("textfile" + str(i) + '.txt', "r+")as temp1_file:
                ...
                Queue_List_TS = re.findall(r'writeq\s*ts.*|readq\s*ts.*|deleteq\s*ts.*', final_cleaned_line,re.IGNORECASE)
                Queue_List_TD = re.findall(r'writeq\s*td.*|readq\s*td.*|deleteq\s*td.*', final_cleaned_line,re.IGNORECASE)
                Other_List = re.findall(r'startbr\s.*|readnext\s.*|readprev\s.*|resetbr\s.*|endbr\s.*',
                                        final_cleaned_line,re.IGNORECASE)
                Operation_List = re.findall(r'read\s.*|write\s.*|rewrite\s.*|delete\s.*', final_cleaned_line,re.IGNORECASE)
                ...
                if result != []:
                    ...
                    for data in result:
                        ...
                        METADATA.append({'component_name': COMPONENT_NAME[:-4], 'component_type': 'COBOL',
                                         'called_name': data,
                                         'called_type': 'MAP','calling_app_name': 'UNKNOWN', 'called_app_name': 'UNKNOWN', 'step_name': '', 'disp': disp_para[0],
                                         'comments': '', 'dd_name': '', 'file_name': ''})
                ...
                if tran_list!=[]:
                    ...
                    METADATA.append({'component_name': COMPONENT_NAME[:-4], 'component_type': 'COBOL',
                                     ...
                                     'called_type': 'TRAN', 'calling_app_name': 'UNKNOWN', 'called_app_name': 'UNKNOWN', 'step_name': '', 'disp': "",
                                     'comments': '', 'dd_name': '', 'file_name': ''})
                ...
                if result1 != []:
                    ...
                    for data in result1:
                        ...
                        METADATA.append({'component_name': COMPONENT_NAME[:-4], 'component_type': 'COBOL',
                                         'called_name': data,
                                         'called_type': 'COBOL', 'calling_app_name': 'UNKNOWN', 'called_app_name': 'UNKNOWN', 'step_name': '',
                                         'disp': disp_para[0],
                                         'comments': '', 'dd_name': '', 'file_name': ''})
                ...
                if Queue_List_TS != []:
                    for data in Queue_List_TS:
                        ...
                        METADATA.append({'component_name': COMPONENT_NAME[:-4], 'component_type': 'COBOL',
                                         'called_name': data,
                                         'called_type': 'TSQ', 'calling_app_name': 'UNKNOWN', 'called_app_name': 'UNKNOWN', 'step_name': '', 'disp': disp_para[0],
                                         'comments': '', 'dd_name': '', 'file_name': ''})
                if Queue_List_TD != []:
                    for data in Queue_List_TD:
                        ...
                        METADATA.append({'component_name': COMPONENT_NAME[:-4], 'component_type': 'COBOL',
                                         'called_name': data,
                                         'called_type': 'TDQ', 'calling_app_name': 'UNKNOWN', 'called_app_name': 'UNKNOWN', 'step_name': '',
                                         'disp': disp_para[0],
                                         'comments': '', 'dd_name': '', 'file_name': ''})
                if Other_List != []:
                    for data3 in Other_List:
                        ...
                        if data!=[]:
                            ...
                            METADATA.append({'component_name': COMPONENT_NAME[:-4], 'component_type': 'COBOL',
                                             'called_name': data,
                                             'called_type': 'FILE', 'calling_app_name': 'UNKNOWN', 'called_app_name': 'UNKNOWN', 'step_name': '',
                                             'disp': '',
                ...
                if Operation_List != []:
                    ...
                    for data3 in Operation_List:
                        ...
                        if not (data3.__contains__("FILE") or data3.__contains__("file") or data3.__contains__("DATASET")):
                            continue
                        disp_para = data3.split()
                        data = re.findall(r'file\s*[(].*[)]', data3,re.IGNORECASE)
                        if data!=[]:
...
def process_cobol_file(COBOL_FILE_LIST1,file_location1,COBOL1):
    ...
    Key_Word_List = ["++INCLUDE", "DISPLAY", "ACCEPT", "INITIALIZE", "EXIT.", "IF", "EVALUATE", "INITIATE", "ADD",
                     ...
                     "STOP", "GOBACK.", "SORT", "MERGE", "EXEC", "ENTRY", "ELSE","RETURN","OPEN","INPUT","OUTPUT","I-O","EXTEND"]
    ...
    for eachFile in COBOL_FILE_LIST1:
                    ...
                    if line.strip().startswith("*") or len(line.strip())==0:
                                    ...
                                    if os.path.isfile(Copyfilepath):
                                        ...
                                        for copylines in tempcopyfile.readlines():
                                            ...
                                            if copylines.strip().startswith("SELECT ") and copylines.strip().endswith('.'):
                                                     ...
                                                     copyfile.write('\n')
                                                     continue
                                            elif copylines.strip().startswith("SELECT ") and not copylines.strip().endswith('.'):
                                                line_flag=True
                        ...
                        if file_flag:
                            ...
                            if line.strip().startswith("SELECT ") and line.strip().endswith('.'):
                                ...
                                if (line.strip().startswith("INPUT ") or line.strip().startswith("OUTPUT ") or line.strip().startswith("I-O ") or line.strip().startswith("EXTEND ")) and not line.split()[1]=="PROCEDURE":
                                   ...
                                   if len(line.strip().split())==2 and line.strip().split()[1].endswith('.'):
                                       ...
                                       continue
                                        ...
                                        if line.strip().startswith("INPUT ") or line.strip().startswith("OUTPUT ") or line.strip().startswith('I-O ')  or line.strip().startswith("EXTEND ") or line.strip.startswith("OPEN "):
        ...
        with open(file_location1 + '\\' + "DataD_lines.CO", 'r') as input_data:
            ...
            for line in lines_list.split("\n"):
                ...
                if (line.strip().startswith('OPEN ') or line.strip().startswith("CLOSE ")) and  (line.strip().split()[1]=="INPUT" or line.strip().split()[1]=="OUTPUT"):
            ...
            for line in lines_list.split("\n"):
                ...
                if not words[1]=="INPUT" and not words[1]=="OUTPUT" and not words[1]=="I-O" and not words[1]=="EXTEND":
