...
def Current_Division(line):
    if (line.split()[0] == "IDENTIFICATION" and line.split()[1] == "DIVISION.") or (line.split()[0] == "ID" and line.split()[1] == "DIVISION."):
...
def Dead_Code(filename):
    ...
    for d in list(Latest_dead_output):
        if d in first_para or d =="MAIN-LOGIC":
            continue
        if d.endswith('-EXIT') or d.endswith('-X') or d.__contains__('-EXIT-' )or d.endswith('-EX')or d.__contains__('-MAINLINE'):
            continue
