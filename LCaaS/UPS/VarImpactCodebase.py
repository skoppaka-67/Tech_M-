...
component_types = {config.codebase_information['COBOL']['folder_name']:config.codebase_information['COBOL']['extension'],
                   config.codebase_information['COPYBOOK']['folder_name']:config.codebase_information['COPYBOOK']['extension'],
                   config.codebase_information['PROC']['folder_name']:config.codebase_information['PROC']['extension'],
                   config.codebase_information['JCL']['folder_name']:config.codebase_information['JCL']['extension'],
                   config.codebase_information['SYSIN']['folder_name']:config.codebase_information['SYSIN']['extension'],
                   config.codebase_information['APS']['folder_name']:config.codebase_information['APS']['extension']
   }

print(component_types)
CopyPath=config.codebase_information['code_location']+'//'+'COPYBOOK'
procpath=config.codebase_information['code_location']+'//'+'PROC'
...
for component in component_types:
    ...
    if component == config.codebase_information['COBOL']['folder_name'] or component == config.codebase_information['COPYBOOK']['folder_name']:
            ...
            for line in file:
                ...
                if len(line)>6:
                    if line[6]=='*':
                        ...
                        OUTPUT_DATA.append(
                            {'component_name': eachFile, 'component_type': component, 'sourcestatements': line.strip()})
                        continue
                OUTPUT_DATA.append({'component_name': eachFile, 'component_type': component, 'sourcestatements': line.strip()})
            ...
            for line in file:
                ...
                if ProcRegexx!=[]:
                    ...
                    if proc_data[2]!="PGM":
                        ...
                        proc_file_path=procpath+'\\'+proc_data_list[0]+'.proc'
                        print("proc",proc_data_list)
                        print(proc_data[2])
                        if os.path.isfile(proc_file_path):

                            Temp_File2 = open(os.path.join(procpath, proc_data[2]+'.proc'), "r")
                            # copy_file.write("#########" + " " + "BEGIN" + " " + line.strip() + '\n')
                            ...
                            for proclines in Temp_File2.readlines():
                                ...
                                if len(line) > 2:
                                    if proclines[2] == '*':
                                        ...
                                        # god_string = god_string + copylines.replace('\n', '<br>')
                                        OUTPUT_DATA.append({'component_name': eachFile, 'component_type': component,
                                                            'sourcestatements': line.strip()})
                                        continue
...
try:
    db.varimpactcodebase.delete_many({'type': {"$ne": "metadata"}})
except Exception as e:
    print('Error:' + str(e))
...
try:
    ...
    db.varimpactcodebase.insert_many(OUTPUT_DATA)
    # updating the timestamp based on which report is called
    current_time = datetime.datetime.now(tz=tz).strftime("%A, %d. %B %Y %I:%M%p")
    if db.varimpactcodebase.update_one({"type": "metadata"}, {"$set": {"last_updated_on": current_time,
                                                                         "time_zone": time_zone,
                                                                         "headers":[
                                                                               "component_name",
                                                                               "component_type",
