...
class XReferenceCobol(Thread):
    ...
    def run(self):
        ...
        if LocalUtils.validateProject(file_location):
            ...
            for eachFile in COBOL_FILE_LIST:
                ...
                try:
                    file = open(file_location + '\\' + COBOL + '\\' + COMPONENT_NAME, 'r', encoding="UTF-8", errors='ignore')
                except:
            ...
            for eachFile in COPYBK_FILE_LIST:
                ...
                COMPONENT_NAME = eachFile
                try:
                    file = open(file_location + '\\' + COPYBOOK + '\\' + COMPONENT_NAME, 'r', encoding="UTF-8", errors='ignore')
                except:
                    print('Could not open\n' + 'File location : ' + file_location)
                ...
                for index, line in enumerate(file):
                    ...
                    try:
                        ...
                        if line[0] == '*':
                            ...
                            # Check if the line conatins a call statement
                            if re.match('^\s*CALL.*', line) or re.match('^\s*call.*', line):
                                # Check if the line contains a direct call
                                if re.match('\'.*\'', line.split()[1]) or re.match('".*"', line.split()[1]):
                                    # Direct call within quotes found, hence adding to metadata directly
                                    METADATA.append(
                            ...
                            try:
                                ...
                                if db.cross_reference_report.count_documents({"type": "metadata"}) > 0:
                                    print('meta happen o naw',
                                          db.cross_reference_report.update_one({"type": "metadata"},
                                                                               {"$set": {
...
def cicsfun(COBOL_FILE_LIST1, file_location1, COBOL1):
    ...
    for eachFile in COBOL_FILE_LIST1:
        ...
        COMPONENT_NAME = eachFile
        print(COMPONENT_NAME)
        file = open(file_location1 + '\\' + DATA + '\\' + COMPONENT_NAME, 'r', encoding="UTF-8", errors='ignore')
        # print("Asdf",file)
        in_exec_flag = False
                ...
                if result != []:
                    ...
                    for data in result:
                        ...
                        if ((data.__contains__('"')) or (data.__contains__("'"))):
                            ...
                            data = data[4:]
                        METADATA.append({'component_name': COMPONENT_NAME[:-4], 'component_type': DATA,
                                         'called_name': data,
...
def process_cobol_file(COBOL_FILE_LIST1, file_location1, COBOL1):
    ...
    Key_Word_List = ["++INCLUDE", "DISPLAY", "ACCEPT", "INITIALIZE", "EXIT.", "IF", "EVALUATE", "INITIATE", "ADD",
                     ...
                     "STOP", "GOBACK.", "SORT", "MERGE", "EXEC", "ENTRY", "ELSE", "RETURN"]
    ...
    for eachFile in COBOL_FILE_LIST1:
        ...
        with open(file_location1 + '\\' + "DataD_lines.CO", 'r') as input_data:
            ...
            for line in lines_list.split("\n"):
                ...
                if line.__contains__('OPEN ') or line.__contains__("CLOSE "):
                    lines_list1 = lines_list1 + line + '\n'
                else:
                    continue

            # print("lines",lines_list1)
            for line in lines_list1.split("\n"):
                words = line.split()
                print(words)
                if words == []:
                    continue
            ...
            for operation in file_operations:
                ...
                db.cross_reference_report.insert_one(oper_dict)
                oper_dict = {}
