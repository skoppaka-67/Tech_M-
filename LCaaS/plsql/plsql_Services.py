...
# client = MongoClient("mongodb+srv://kiran:kiranaws@cluster0.w58py.mongodb.net/plsql?retryWrites=true&w=majority")
...
def deadparalist():
    ...
    try:
        ...
        cursor = db.master_inventory_report.find(
            ...
            {"component_name": 1, "component_type": 1, "dead_para_list": 1, "dead_para_count": 1, "no_of_dead_lines": 1,
             "_id": 0})
...
def procedureFlowCompList():
    ...
    if option is not None:
        ...
        try:
            response = db.master_inventory_report.find({"application": application_name,"component_type":"Package Body"})
            if response is None:
...
def bre():
    ...
    try:
        ...
        option = option.split('.')
        option = option[0]
        cursor = db.bre_rules_report.find({"pgm_name": option}, {"_id": 0})
...
def procFlowChart():
    ...
    component_name = request.args.get('component_name')+".pkb"
    try:
...
def getCode():
    ...
    if not re.match('\S+\.\S+', component_name.strip()):
        ...
        master_component_types = {'CODEBEHIND': 'aspx.vb', 'JAVASCRIPT': 'js', 'WEBFORM': 'aspx',
                                  'VB': 'vb'}  # This line to be copied from the script
    ...
    cursy = db.codebase.find_one({'component_name': component_name}, {"_id": 0})
    end = timeit.default_timer()

    # if component_line != None:
    #     replaced_lines = ""
    #     for k in cursy["code_string"].split("<br>"):
    #         if k.__contains__(component_line):
    #             replaced_lines = replaced_lines + '<span id=canvasScroll>' + component_line + '<br>' + "</span>"
    #         else:
    #             replaced_lines = replaced_lines + k + "<br>"
    #     cursy["code_string"] = replaced_lines
    # print(component_line)
    # print(cursy["codeString"])
...
def chartsAPI():
    ...
    try:
        ...
        total_loc = db.master_inventory_report.aggregate(
            [{"$match": { "component_type" :{"$ne": "PROCEDURE"}}}, {"$group": {"_id": "null", "count": {"$sum": "$Loc"}}}])
...
def cycComplex():
    ...
    application = []
    orphan_data1 = []
    cursor = db.master_inventory_report.find({"$or":[{"component_type": "Package Body"},{"component_type": "Package Specification"}]})
...
def buisnessConnected():
    ...
    #     cbr_count = 0
    #     for each in val:
    #         query = db.bre_rules_report.find({'$and': [ {"rule_category": "Business Rule"}]}).count()
    #         query1 = db.bre_rules_report.find(
    #             {'$and': [ {"rule_category": "Connected Business Rule"}]}).count()
    #         br_count = br_count + query
    #         cbr_count = cbr_count + query1
    #     # print('final_key',br_count,key1)
    #     # print('final_key', cbr_count, key1)
...
#     whole_key=[]
#     for k,k1 in zip(ordered_dict_inbound.keys(), ordered_dict_outbound.keys()):
#         whole_key.append(k)
#         whole_key.append(k1)
#     in_key=list(ordered_dict_inbound.keys())
#     out_key= list(ordered_dict_outbound.keys())
#     print(in_key, out_key)
#     print(whole_key, list(ordered_dict_outbound.keys()))
#     for key in whole_key:
#         if(key not in in_key):
#             # print(key)
#             ordered_dict_inbound={key:''}
#         if(key not in out_key):
#             print("outbound",key, ordered_dict_outbound)
#             ordered_dict_outbound[key]= ''
#         # print(k,v)
#
...
#         print(ordered_dict_outbound[k], ordered_dict_inbound[k1])
#         in_data.append({k1:ordered_dict_inbound[k1]})
#         out_data.append({k:ordered_dict_outbound[k]})
    ...
    try:
        ...
        if 'data' in keys:
            ...
            for program in program_list:
                ...
                temp['component_name'] = program.split(".")[0]
                node_set = set()
                link_set = set()
                ...
                for pgm_name in payload['data'][program]:
                    ...
                    if pgm_name['name'] == "TABLE":
                        ...
                        link_set.add(json.dumps(
                            {"source": 'p_' + from_node, "target": 'p_' + to_node, "label": 'TABLE'}))
        ...
        for i in connections:
            ...
            if i["called_type"].lower() != "utility":
                ...
                response["nodes"].append(
                    {"id": "p_" + i["component_name"], "label": i["component_name"] + "(" + i["component_type"] + ")"})
                response["nodes"].append(
...
#     #                                 if connection['called_type'].lower() != 'file' and\
#     #                                         connection['called_type'].lower() != 'workbook':
#     #                                     if(connection['called_name'] not in component_name):
#     #                                         # print(component_name)
...
@app.route('/api/v1/callChainFilter', methods=['GET'])
def callchaindropdown():
    response={}
    response['component_name']=request.args.get('component_name')
    response['component_type'] = request.args.get('component_type')
    not_include= response['component_type']
                        ...
                        if connection['called_name'].strip() != '':
                            ...
                            if connection['component_name'].strip() == input_name:
                                    ...
                                    if (connection['component_name'] not in component_name):
                                        component_name.append(connection['component_name'])
                                        component_type.append(connection['component_type'])
        component_type1=list(set(component_type))
        component_type1.insert(0,"All")
        if(component_type1.count(not_include)==1):
            component_type1.remove(not_include)
        return jsonify(component_type1)
    except:
        pass



@app.route('/api/v1/callchainplsql', methods=['GET'])
def callchain():
    source_child = []
    source_parent = []
    spider_details = set()
                        ...
                        if connection['called_name'].strip() != '':
                            ...
                            if connection['component_name'].strip() == input_name:
                                if parent_child == "child":
                                        ...
                                        #         flag = 0
                                        #         print("flag1", flag)
                                        # print("C1_type", c_type)
                                            if (connection['called_name'] not in component_name):
                                                # print(component_name)
                                                if (response['option'] == "All"):
                                                    print(connection['called_type'], connection['called_name'])
                                                    component_name.append(connection['called_name'])
                                                    component_type.append(connection['called_type'])
                                                    child_output = child_link(connection['component_name'],
                                                                              connection['called_name'])
                                                ...
                                                if (response['option'] == connection['called_type']):
                                                    print(connection['called_type'], connection['called_name'])
                                                    component_name.append(connection['called_name'])
                                                    component_type.append(connection['called_type'])
                                                    child_output = child_link(connection['component_name'],
                                ...
                                if (parent_child == "parent"):
                                        # print("C1.1_type", c_type)
                                        # parent_output = parent_link(connection['called_name'],
                                        #                             connection['component_name'])
                                        # print(parent_output)
                                        if (connection['component_name'] not in component_name):
                                            if (response['option'] =="All"):
                                                print(connection['called_type'])
                                                component_name.append(connection['component_name'])
                                                component_type.append(connection['component_type'])
                                                parent_output = parent_link(connection['called_name'],
                                                                            connection['component_name'])
                                                name_type_map[connection['component_name']] = connection[
                                                    'component_type']
                                                if connection['component_name'] not in nodes:
                                                    nodes.add(connection['component_name'])
                                                    flag = 0

                                            if (response['option'] == connection['component_type']):
                                                print(connection['called_type'], connection['component_name'])
                                                component_name.append(connection['component_name'])
                                                component_type.append(connection['component_type'])
                                                parent_output = parent_link(connection['called_name'],
                                                                            connection['component_name'])
...
def updateBRE():
    try:
        ...
        if db.bre_report3.update_one({"fragment_id": fragment_id}, {
...
def bre_2():
    ...
    try:
        ...
        option = option[0]
        # print(option)
        cursor = db.bre_report3.find({"pgm_name": option}, {"_id": 0}).sort('Rule')
        datavalue["headers"] = header
...
def procFlowChart_RD():
    ...
    try:
        flowchart_metadata = db.translated_flowchart.find_one(
            {"$and": [{"component_name": component_name+".pkb"}, {"para_name": para_name}]}, {'_id': 0})
...
def comment_line_report():
    ...
    # print(cursors)
    if apname == "All":
        cursors = db.cobol_output.find({"codeString": {"$ne": ""}})
        for cursor in cursors:
            json_format = {"component_name": cursor["component_name"],
                           "component_type": cursor["component_type"]}
        ...
        cursors = db.cobol_output.find({"application": apname, "codeString": {"$ne": ""}})
        for cursor in cursors:
            json_format = {"component_name": cursor["component_name"],
                           "component_type": cursor["component_type"]}
            if (json_format not in output_list):
...
@app.route('/api/v1/cicsxref', methods=['GET'])
def cicsxref():
    data_object = {}
    data_object['data'] = []
    try:
...
def cicsrule():
    ...
    try:
        option = request.args.get('option')
        header = ["program_name", "map_name", "field_name", "validation_rule"]
        option = option.split('.')
        option = option[0]
        ...
        try:
            if(connection['rule_category']==bre_connection):
                if (search_filter != '' ):
                    ...
                    out_list.append({'pgm_name': connection['component_name'],"fragment_Id":connection['fragment_Id'], 'para_name':connection['procedure_name'], "source_statements":connection["source_statements"], "rule_category":connection["rule_category"], "parent_rule_id":connection["parent_rule_id"], "business_documentation":connection["business_documentation"]})
...
def package():
    ...
    try:
        if option=="PACKAGE":
            list_of_programs = db.bre_rules_report.distinct("package_name")
            for pgm in list_of_programs:
                pgm_name_set.add(pgm)
        elif option=='STAND-ALONE':
            list_of_programs=['PROCEDURE',"TRIGGER","FUNCTION"]
            for pgm in list_of_programs:
                pgm_name_set.add(pgm)
        return jsonify({"package": sorted(list(pgm_name_set))})
    except Exception as e:
        print('Error:', str(e))
        return jsonify({"status": "failure", "reason": "Error: " + str(e)})

@app.route('/api/v1/breplsql', methods=['GET'])
def breplsql():
   datavalue={}
   datavalue["data"]=[]
   ...
   try:
    ...
    header= [ 'component_name',"component_type","procedure_name", 'fragment_Id',
                'source_statements', 'rule_category', 'parent_rule_id'
                ]

    if option1=="FUNCTION" and option=="STAND-ALONE":
        # header = ['function_name', 'fragment_Id',
        #           'source_statements', 'package_name', 'rule_category', 'parent_rule_id'
        #           ]
        cursor = db.bre_rules_report.find({"$and":[{"component_type":option1,"component_name": option2}]},{"_id":0})
        datavalue["headers"]=header
        for record in cursor:
            datavalue["data"].append(record)

    elif  option1=="PROCEDURE"  :
        # header = [ 'procedure_name',  'fragment_Id',
        #           'source_statements', 'package_name', 'rule_category', 'parent_rule_id'
        #           ]
        cursor = db.bre_rules_report.find({"$and":[{"component_type":option1,"component_name": option2}]},{"_id":0})
        datavalue["headers"]=header
    ...
    elif  option1=="TRIGGER"  :
        # header = ['trigger_name',  'fragment_Id',
        #           'source_statements', 'package_name', 'rule_category', 'parent_rule_id'
        #           ]
        ...
        header = ["procedure_name", 'fragment_Id',
                  'source_statements', 'rule_category', 'parent_rule_id',"package_name"
                  ]

        # cursor = db.bre_rules_report.find({"$and":[{'package_name':option1,"procedure_name": option2}]},{"_id":0})
        cursor=db.bre_rules_report.find({'package_name':option1},{"_id":0})
        datavalue["headers"] = header
        for record in cursor:
            datavalue["data"].append(record)

        # if datavalue["data"]=="":
        #     cursor = db.bre_rules_report1.find({"$and": [{'package_name': option1, "function_name": option2}]},{"_id":0})
        #     datavalue["headers"] = header
        #     for record in cursor:
        #         datavalue["data"].append(record)
        #
        # if   datavalue["data"]=="":
        #    cursor = db.bre_rules_report1.find({"$and": [{'package_name': option1, "trigger_name": option2}]},{"_id":0})
        #    datavalue["headers"] = header
        #    for record in cursor:
        #        datavalue["data"].append(record)
...
@app.route('/api/v1/package_list', methods=['GET'])
def package_list():
    datavalue = {}
    datavalue["data"] = []
    package_name = set()
        ...
        if option=="PACKAGE":
            ...
            for data in cursor:
                if data["procedure_name"]!="":
                    package_name.add(data["procedure_name"])
                elif data["function_name"] != "":
                    package_name.add(data["function_name"])
                elif data["trigger_name"] != "":
                    package_name.add(data["trigger_name"])

        elif option=="STAND-ALONE":
            cursor = db.bre_rules_report.find({"package_name": ""}, {"_id": 0})
            for data in cursor:
                if option1=="FUNCTION":
                     if data['component_type']=="FUNCTION" and data["package_name"]=="":
                         package_name.add(data["component_name"])
                elif option1=="PROCEDURE":
                     if data['component_type']=="PROCEDURE" and data["package_name"]=="":
                         package_name.add(data["component_name"])
                elif option1=="TRIGGER":
                     if data['component_type']=="TRIGGER" and data["package_name"]=="":
                         package_name.add(data["component_name"])

        return jsonify({"package_list": sorted(list(package_name))})
    except Exception as e:
        print('Error:', str(e))
        return jsonify({"status": "failure", "reason": "Error: " + str(e)})


@app.route('/api/v1/breplsql2', methods=['GET'])
def breplsql2():
   datavalue={}
   datavalue["data"]=[]
   ...
   try:
    ...
    if option1=="FUNCTION" :
        ...
        #           'source_statements', 'package_name', 'rule_category', 'parent_rule_id'
        #           ]
        cursor = db.bre_report2.find({"$and":[{"component_type":option1,"component_name": option2}]},{"_id":0})
        datavalue["headers"]=header
        for record in cursor:
            datavalue["data"].append(record)
        ...
        cursor = db.bre_report2.find({"$and":[{"component_type":option1}]},{"_id":0})
        datavalue["headers"]=header
        for record in cursor:
            datavalue["data"].append(record)
        ...
        cursor = db.bre_report2.find({"$and":[{'package_name':option1}]},{"_id":0})
        #cursor=db.bre_rules_report1.find({'package_name':option1},{"_id":0})
        datavalue["headers"] = header
        for record in cursor:
            datavalue["data"].append(record)
...
def inout():
    ...
    connections=db.cross_reference_report_copy.find({"type":{'$ne':'metadata'}})
    calling_list=[]
    called_list=[]
    d={}
    metadata=[]
