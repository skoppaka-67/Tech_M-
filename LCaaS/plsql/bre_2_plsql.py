...
mongoClient_kiran = MongoClient('172.18.33.227', 27017)
mongoClient_kiran = MongoClient('localhost', 27017)


db_kiran = mongoClient_kiran['plsql']
# db_kiran = mongoClient_mothi['GE_CICS']
h = {"type": "metadata", "headers":["fragment_id","pgm_name", "para_name", "source_statements","rule_category", "Rule"]}
db_kiran.bre_report2.remove()
db_kiran.bre_report2.insert_one(h)
bre_json_objects = []
Meatadata = []
flag = False
storage = []
...
def main():
    ...
    collection = db_kiran.bre_rules_report
    ...
    for i in range(1,sheet.nrows):
      ...
      try:
        if j < sheet.nrows:
            ...
            if sheet.cell_value(i, 10) != sheet.cell_value(j, 10):
                # new_json.clear()
                if sheet.row_values(i)[2] == "" and sheet.row_values(i)[3] == "" and  sheet.row_values(i)[4] == "":

                    new_json["para_name"] = sheet.row_values(i)[8]#package
                    """
                    package - 8
                    procedure - 2
                    func - 3
                ...
                elif sheet.row_values(i)[2] == "" and sheet.row_values(i)[4] == "":
                    new_json["para_name"] = sheet.row_values(i)[3]#function
                elif sheet.row_values(i)[3] == "" and sheet.row_values(i)[4] == "":
                    new_json["para_name"] = sheet.row_values(i)[8]  # procedure
                elif sheet.row_values(i)[3] == "" and sheet.row_values(i)[8] == "":
                    new_json["para_name"] = sheet.row_values(i)[4]  #trigger


                new_json['pgm_name'] = sheet.row_values(i)[8]
                new_json['source_statements'] = new_json['source_statements'] + " <br> " + str(storage).strip("[]").replace(",","").replace('"',"")

                new_json['parent_rule_id'] = sheet.row_values(i)[10]
            ...
            if flag:
                storage.append(sheet.row_values(j)[7] + " <br> ")
                continue
            ...
            if sheet.cell_value(i, 10) == sheet.cell_value(j, 10):
                ...
                new_json['pgm_name'] = sheet.row_values(i)[8]
                new_json['source_statements'] = sheet.row_values(i)[7] + " <br> " + sheet.row_values(j)[7]
                new_json["rule_description"] = ""
                flag = True
        else:
          ...
          new_json['source_statements'] = sheet.row_values(i)[7]
          new_json["rule_description"] = ""
          new_json['parent_rule_id'] = sheet.row_values(i)[10]
          storage.clear()
          new_json1 = copy.deepcopy(new_json)
    ...
    try:
        db_kiran.bre_report2.delete_many({'type': {"$ne": "metadata"}})
    ...
    try:
        ...
        db_kiran.bre_report2.insert_many(matadata)

        # updating the timestamp based on which report is called
        current_time = datetime.datetime.now(tz=tz).strftime("%A, %d. %B %Y %I:%M%p")
        if db_kiran.bre_report2.update_one({"type": "metadata"}, {"$set": {"last_updated_on": current_time,
                                                                     "time_zone": time_zone,
