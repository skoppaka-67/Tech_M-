...
for ite in res:
    try:
        if ite['_id']!= {}:
            ...
            # print("master inventory", master_inv_set)
    except:
        pass


# Fetch all the items in X-Reference called components
distinctIdCode = { "$group": { "_id": { "called_name": "$called_name", "called_type": "$called_type","comments":"$comments" } } }
...
for ite in res:
    ...
    if ite['_id'] != {}:
        #print(ite['_id'])
        if ite['_id']['called_type'] not in ignored_types_list and ite['_id']['comments']!='inline':
