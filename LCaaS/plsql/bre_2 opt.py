...
def InsterHeaders():
    try:
        ...
        h = {"type": "metadata",
             "headers": ["fragment_id", "package_name", "procedure_name", "source_statements", "rule_category", "Rule"]}
...
def main():
    ...
    # InsterHeaders()
    compnentname_list = db.bre_rules_report.distinct("package_name")
    # compnentname_list = ['ACSSNAVS.']
    bre_complist = db.bre_report2.distinct("package_name")
    ignore_list = ['NA20AAAK.','NA20AAAL.']

    for component in compnentname_list:
     if component != None and len(component) > 3:
        ...
        details = collection.find({"$and":[{"package_name": component},
                                            {"parent_rule_id" :{"$ne": ""}},
            ...
            try:
                if j < len(bre_json_objects):
                    ...
                    if bre_json_objects[i]['parent_rule_id'] != bre_json_objects[j]['parent_rule_id']:
                        ...
                        new_json['package_name'] = bre_json_objects[i]['package_name'].replace(".","")
        ...
        for index,i in enumerate(Meatadata,1):
                ...
                pgmname = i["package_name"].replace(".","")
                paraname = i['procedure_name']
                source_statements = i['source_statements']
                Rule = i['parent_rule_id']
                rule_relation = ""
                # print(bre2_json)
                bre2_json = {"fragment_id": component.replace('.','')+"-"+str(index),"package_name": pgmname.replace(".",""), "procedure_name": paraname, "source_statements": source_statements,'rule_category':" ", 'Rule': Rule,
