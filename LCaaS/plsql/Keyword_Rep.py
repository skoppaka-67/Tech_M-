...
def main():
    ...
    for bre_i in bre_json_objects:
        try:
            ...
            for dict in key_word_json_object:
                if bre_i['rule_description'].__contains__(dict['keyword']) or bre_i['rule_description'].__contains__(dict['keyword'].upper()):
                ...
                # Upper case
                # """
                # if bre_i['rule_description'].__contains__(dict['keyword'].upper()):
                #     replaced1 = re.sub(" "+ dict['keyword'].upper()+" ", " "+dict['Meaning']+" ", bre_i['rule_description'],
                #                        flags=re.IGNORECASE) #sourcestatements will starts with space so every cobol keyword will have one space before
                #     bre_i['rule_description'] = replaced1
    ...
    for flo_i in flowchart_json_objects:
        for dict in key_word_json_object:
            if dict['keyword']=="START" or dict['keyword']=="End":
                replaced1 = re.sub( dict['keyword'], " " + dict['Meaning']+" ", flo_i['option'][17:], flags=re.IGNORECASE)
                flo_i['option'] = flo_i['option'] [:50] +replaced1
                # print(flo_i['option'])

            else:
                replaced1 = re.sub(dict['keyword'], " " + dict['Meaning']+" ", flo_i['option'], flags=re.IGNORECASE)
