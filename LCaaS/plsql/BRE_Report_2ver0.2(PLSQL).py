...
SCRIPT_VERSION = "BRE-Report2- Author - PLSQL   "


mongoClient = MongoClient('localhost', 27017)
db = mongoClient['plsql']
h = {"type": "metadata", "headers": ["pgm_name", "para_name", "source_statements", "Rule"]}
...
def main():
    ...
    for i in Meatadata:
            ...
            rule_relation = ""
            # print(bre2_json)
            bre2_json = {"pgm_name": pgmname, "para_name": paraname, "source_statements": source_statements, 'Rule': Rule,
    ...
    for filename in glob.glob(os.path.join(RPG_Path, '*.cbl')):

        #print(filename.strip("D:\AS400\RPG\\").strip("."))
        compnentname =  filename.strip("D:\\Input\\COBOL333\\").strip(".")
        print("com",compnentname)
        index = 1
        #for index, item in enumerate(no_dup_list, start=1):
        for item in no_dup_list:

            current_compnentname = item.split("-")[0] + ".cbl"
