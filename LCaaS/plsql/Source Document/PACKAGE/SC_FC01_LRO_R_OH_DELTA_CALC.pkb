CREATE OR REPLACE PACKAGE BODY CRPSC.SC_FC01_LRO_R_OH_DELTA_CALC
AS
   /**********************************************************************************************
           ...
           || Crated a New Package to Proces the Onhand Inventory from FC01 Feed - Receipts Inventory. hkommoju
            ...
            || March 15, 2017   Hanock Kommoju (hkommoju) Made Changes to FC01 In Transit Refresh - Join condition to Delivery Line Table.
            ||-----------------------------------------------------------------------------------------
        **********************************************************************************************/
   ...
   BEGIN
      ...
      SC_FC01_LRO_R_OH_STG;

      SC_FC01_LRO_R_OH_HIST;

      SC_FC01_LRO_R_OH_DELTA;
   ...
   BEGIN
      ...
      -- FC01 - In Transit - Inventory  Process:

      --        DELETE FROM SC_FC01_SNAPSHOT_LRO;
      --
      --        INSERT INTO SC_FC01_SNAPSHOT_LRO
      --        --
      --        SELECT
      --        PL_PURCHASE_ORDER_NUMBER,
      --        PL_LINE_NUMBER,
      --        PL_END_USER_PRODUCT_ID,
      --        PL_PARTNER_ID,
      ...
      --        PD_CREATION_DATE,
      --        PD_LAST_UPDATE_DATE,
      --        PD_DELIVERY_LINE_NUMBER,
      --        PD_CURRENT_DELIVERY_DATE,
      --        PD_REQUIRED_QTY,
      --        PD_TOTAL_RECEIVED_QTY,
      --        PD_REMAINING_QTY,
      --        REFRESH_PART_NUMBER,
      --        ROHS_FLAG,
      --        RECORD_CREATED_ON,
      ...
      --        From
      --        (
      --            select  po.status_code PO_status_code, pl.status_code PL_status_code, pd.status_code PD_status_code,
      ...
      --            po.BUYER PO_BUYER
      --            from IF_PURCHASE_ORDER_LINE@sc_esmprd pl   -- 524
      --            Inner Join IF_PURCHASE_ORDER@sc_esmprd po
      --                on ( pl.TRANSACTION_ID = po.TRANSACTION_ID and pl.PLANNING_DIVISION = po.PLANNING_DIVISION and pl.PURCHASE_ORDER_NUMBER = po.PURCHASE_ORDER_NUMBER )
      --            Inner Join IF_PURCHASE_ORDER_DELIVERY@sc_esmprd pd
      --                on ( pl.TRANSACTION_ID = pd.TRANSACTION_ID and pl.PLANNING_DIVISION = pd.PLANNING_DIVISION and pl.PURCHASE_ORDER_NUMBER = pd.PURCHASE_ORDER_NUMBER and pl.LINE_NUMBER = pd.LINE_NUMBER )
      --            inner join crpadm.rc_product_master pm
      --                on (pl.END_USER_PRODUCT_ID  = pm.tan_id)
      --            where 1=1
      --            --and po.LOAD_TYPE = 'INCR'
      --            and pl.PLANNING_DIVISION in ('LRO')
      --            and pd.TRANSACTION_ID =  (select max(ipd.TRANSACTION_ID) from IF_PURCHASE_ORDER_DELIVERY@sc_esmprd ipd
      --                                        where ipd.PLANNING_DIVISION = pl.PLANNING_DIVISION
      --                                        --and ipd.END_USER_PRODUCT_ID = pl.END_USER_PRODUCT_ID
      --                                        and ipd.PURCHASE_ORDER_NUMBER = pl.PURCHASE_ORDER_NUMBER
      --                                        and ipd.LINE_NUMBER = pl.LINE_NUMBER
      --                                    )
      --        ) ;
      ----        --where PO_status_code = 'Open' and PL_status_code = 'Open' and  PD_status_code = 'Open' and PD_REMAINING_QTY > 0 ;
      ----
      --     Commit;
      --
      --     -->>  FC01 - Receipts - On Hand Inventory Process
      --
      --      Delete from SC_FC01_OH_STG_LRO  where refresh_part_number like '%RF';  -->> Added Newly.
      --
      ...
      --                                    Group By ipr.Purchase_Order_Number, ipr.LINE_NUMBER, ipr.DELIVERY_LINE_NUMBER, ipr.transaction_id, ipr.RECEIVED_DATE --CREATION_DATE, LAST_UPDATE_DATE, RECEIVED_DATE
      --                                ) pr
   ...
   EXCEPTION
      ...
      THEN
         ...
         COMMIT;
   END SC_FC01_LRO_R_OH_STG;

   /*-----------------------Procedure to populate history table SC_FC01_OH_HIST_LRO from SC_FC01_OH_STG_LRO table-------------------------------*/
           ...
           SELECT A.*, ROWNUM
             FROM (  SELECT PRODUCT_ID,
                   ...
                   ORDER BY PRODUCT_ID,
                            ...
                            REFRESH_PART_NUMBER,
                            ROHS_FLAG,
                            RECEIVED_DATE) A -->> included received_date column as part of sc integration
         ORDER BY ROWNUM;
   ...
   BEGIN
      ...
      LOOP
         ...
         THEN
               ...
               BEGIN
                  ...
                  UPDATE SC_FC01_OH_HIST_LRO
                     SET RUN_STATUS =
                         ...
                         RECORD_UPDATED_BY = 'SC_FC01_LRO_R_OH_DELTA_CALC' -->> Need to check
                   WHERE     UPPER (PRODUCT_ID) =
                         ...
                         AND UPPER (ROHS_FLAG) =
                                UPPER (FC01_INV_RECORD.ROHS_FLAG) -->> Added New
                         AND RECEIVED_DATE = FC01_INV_RECORD.RECEIVED_DATE -->> included received_date column as part of sc integration
            ...
            ELSE
               ...
               --DBMS_OUTPUT.PUT_LINE ( 'FC01_INV_RECORD.PRODUCT_ID:' || FC01_INV_RECORD.PRODUCT_ID);
               --DBMS_OUTPUT.PUT_LINE ( 'FC01_INV_RECORD.PO_NUMBER:' || FC01_INV_RECORD.PO_NUMBER);


               SELECT MAX (TRANSACTION_ID)           -->> Changed to Max Newly
               ...
               THEN
                  UPDATE SC_FC01_OH_DELTA_LRO
                     SET RECEIVED_QTY = VAR_QTY,
                         RECORD_UPDATED_ON = SYSDATE,
                         RECORD_UPDATED_BY = 'SC_FC01_LRO_R_OH_DELTA_CALC',
                         NEW_REC = 'Y'
                   WHERE     UPPER (PRODUCT_ID) =
                                UPPER (FC01_INV_RECORD.PRODUCT_ID)
                         AND UPPER (PO_NUMBER) =
      ...
      COMMIT;
   END SC_FC01_LRO_R_OH_DELTA;
END SC_FC01_LRO_R_OH_DELTA_CALC;
/