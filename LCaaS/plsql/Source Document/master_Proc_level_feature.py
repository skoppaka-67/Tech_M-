...
def procedures(files):
    ...
    for line in f.readlines():  ##access lines one by one from file
        ...
        if line.strip().startswith('PROCEDURE'):  ##access lines which are starting with procedure
            ...
            procedure_name = a[1]  ##line starts with procedure and the next word will be the procedure name, so we are storing a[1] in procedure name

        if flag and line.strip().startswith('END' + ' ' + procedure_name):  ##access the lines which are starting with end followed by procedure name
        ...
        if flag:  ##if line starts with procedure flag will be true
            ...
            5. we update all the information in output dictionary and append it to metadata
            """
            storage.append(line)
...
function to read master inv data from DB 
