...
@Component({
    selector: 'app-masterinvapp',
    templateUrl: './masterinv-application.component.html',
    styleUrls: ['./masterinv-application.component.scss'],
    animations: [routerTransition()]
})

export class MasterinvAppComponent implements OnInit {
  constructor(
  ...
  ngOnInit() {
    ...
    this.getMasterInvenDetailsWithApplicationFilter('', true);
    this.getLoadedValues();
    // this.getComponentTypeListMaster();
     // -- 1 -- this.getMasterInvenDetails();
...
applicationTypeOnchange(event: any) {
  ...
  // console.log(inputJson[0].appName);
}
...
onSubmit(){
  ...
  if(this.selectedApplication.indexOf('&')!=-1){
    this.selectedApplication = this.selectedApplication.replace('&','$');
      ...
      this.getMasterInvenDetailsWithApplicationFilter(this.selectedApplication, false);
      this.showLoader = false;
      setTimeout(() => {
        $( $('#masterinv').DataTable().column(0).nodes()).addClass('highlight');
    }, 1100);
}
getMasterInvenDetailsWithApplicationFilter(selectedApplication, initializeTable: boolean) {
  if (initializeTable) {
    this.showLoader = true;
    if(this.selectedApplication.indexOf('&')!=-1){
      this.selectedApplication = this.selectedApplication.replace('&','$');
    }
    this.dataservice.getMasterInvenDetailsWithApplicationFilter(this.selectedApplication).subscribe(res => {
      this.dataSets = res;
      var dataJSON = {};
            var dataList = [];
