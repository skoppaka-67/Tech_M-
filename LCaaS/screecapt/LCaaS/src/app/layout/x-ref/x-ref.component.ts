...
@Component({
    selector: 'app-x-ref',
    templateUrl: './x-ref.component.html',
    styleUrls: ['./x-ref.component.scss'],
...
export class XrefComponent implements OnInit {
    ...
    exportAsXLSX(): void {
       this.dataservice.getXRefDetails('', 'yes').subscribe(res => {
        //  var res = {"headers" : ["A", "B"], "data" : [{"B": "b1", "A":"a1"}, {"A": "a2", "B":"b2"}] }
            var dataJSON = {};
            var dataList = [];
       ...
       /*
       this.excelDataChanges = res.data;
            this.excelHeaders = res.headers;

            for(var i=0;i<this.excelDataChanges.length;i++) {
                var eachJSONKeys = Object.keys(this.excelDataChanges[i]);
                var removeKeyArray = this.excelHeaders.filter(function(obj) { return eachJSONKeys.indexOf(obj) == -1; });
            }
            for(var i=0;i<this.excelDataChanges.length;i++) {
                removeKeyArray.forEach(function(key){
                    delete this.excelDataChanges[i][key];
                });
             }
             console.log(this.excelDataChanges);     
             console.log('Excel Data Check')        
            this.excelDataSet = this.excelDataChanges;
        */
    filterTable() {
