...
@Component({
    selector: 'app-breplsql-app',
    templateUrl: './brereportplsql-app.component.html',
    styleUrls: ['./brereportplsql-app.component.scss'],
    animations: [routerTransition()]
})

export class BreReportPlSqlAppComponent implements OnInit {
  ...
  componentList: any[] = [];
   ...
   dataSets: any = null;
    ...
    applicationTypeOnchange(event: any){
      ...
      ( document.getElementById('fileType') as HTMLInputElement ).value = '';
      ( document.getElementById('list') as HTMLInputElement ).value = '';
      ( document.getElementById('pgmName') as HTMLInputElement ).value = '';
    }
    // getComponentList() {
    ...
    fileTypeOnChange(event: any){
      this.getList(event.target.value, this.selectedApplication);
      this.selectedType = event.target.value;
      ( document.getElementById('list') as HTMLInputElement ).value = '';
      ( document.getElementById('pgmName') as HTMLInputElement ).value = '';
    }
    listOnChange(event: any){
      this.getProgramNames(event.target.value, this.selectedApplication);
    ...
    // });
    // }
    getProgramNames(selectedList, selectedApplication) {
      this.dataservice.getProgramNamesWithApp(this.selectedType, selectedList, selectedApplication).subscribe(res => {
        this.programNames = res.package_list.filter(Boolean);
    });
    }
    getList(selectedType, selectedApplication){
      this.dataservice.getListWithApp(selectedType, selectedApplication).subscribe(res => {
    ...
    onSubmit() {
      ...
      this.getBREDetailsNew(this.selectedType, this.selectedList, this.selectedProgram, this.selectedApplication, false);
      this.showLoader = false;
    }
    getBREDetailsNew(selectedType, selectedList, selectedProgram, selectedApplication,intializeTable: boolean) {
      if (intializeTable) {
      this.dataservice.getBREPlSql2DetailsNewWithApp(selectedType, selectedList, 
                  selectedProgram, selectedApplication).subscribe(res => {
