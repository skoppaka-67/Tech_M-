...
@Component({
    selector: 'app-cics-x-ref-app',
    templateUrl: './cicsxref-application.component.html',
    styleUrls: ['./cicsxref-application.component.scss'],
    animations: [routerTransition()]
})
export class CICSXrefAppComponent implements OnInit {
    constructor(
    ...
    onSubmit(){
      if(this.selectedApplication.indexOf('&')!=-1){
        ...
        this.getCICSXRefApplicationDetails(this.selectedApplication, '', 'no');
    }
    getCICSXRefApplicationDetails(selectedApplication, searchFilter, flag){
        ...
        this.dataservice.getCICSXRefAppDetails(selectedApplication, '', 'no').subscribe(res => {
            this.dataSets = res;
    ...
    exportAsXLSX(): void {
      if(this.selectedApplication.indexOf('&')!=-1){
        ...
        this.dataservice.getCICSXRefAppDetails(this.selectedApplication, '', 'yes').subscribe(res => {
            var dataJSON = {};
            var dataList = [];
            var headerList = res.headers;
            for(var i=0;i<res.data.length;i++){
          ...
          if(this.selectedApplication.indexOf('&')!=-1){
            ...
            this.excelService.exportAsExcelFile(dataList, this.selectedApplication + '_CICS X-Ref Report');
        });
       }
    ...
    filterTable() {
        ...
        if (this.searchFilter.length > 2) {
            ...
            // this.dataservice.getXRefDetails(this.searchFilter, 'no').subscribe(res => {
              ...
              this.dataservice.getCICSXRefAppDetails(this.selectedApplication, this.searchFilter, 'no').subscribe(res=> {
                this.dataSets = res;
                // (document.getElementById('totalrecords') as HTMLInputElement).innerHTML = 'Total no of records: ' + res.data.length;
                if(res.total_record_count){
                    (document.getElementById('totalrecords') as HTMLInputElement).innerHTML = 'Total no of records: ' + res.total_record_count;
