...
@Component({
  selector: 'app-bre',
  templateUrl: './glossary-application.component.html',
  styleUrls: ['./glossary-application.component.scss'],
  animations: [routerTransition()]
})

export class GlossaryAppComponent implements OnInit {
  ...
  onSubmit(){
    this.getGlossaryDetails(this.selectedApplication, this.selectedComponent, false);
  }
  getGlossaryDetails(selectedApplication, selectedComponent, intializeTable: boolean) {
    if (intializeTable) {
      this.showLoader = true;		
      // this.dataservice.getGlossaryDetails().subscribe(res => {	
        // this.dataservice.getGlossaryDetails_limit("no").subscribe(res => {	
          this.dataservice.getGlossaryDetails_limit_app(selectedApplication, selectedComponent, "no").subscribe(res => {	
          this.dataSets = res;		
  ...
  exportAsXLSX(): void {
    // this.dataservice.getGlossaryDetails_limit("yes").subscribe(res => {	
      this.dataservice.getGlossaryDetails_limit_app(this.selectedApplication, this.selectedComponent, "yes").subscribe(res => {	
      // this.dataservice.getGlossaryDetails().subscribe(res => {		
        var dataJSON = {};
        var dataList = [];
        var headers = res.headers;
        ...
        this.excelService.exportAsExcelFile(dataList, 'Glossary Report');
   });
  }
  open(content) {
  ...
  updateVariableInfo(){
    ...
    // this.dataservice.updateVariableDefinition().subscribe(res => { //for other instances
    // for crst
      this.dataservice.updateVariableDefinitionWithApp(this.selectedApplication, this.selectedComponent).subscribe(res => {
      console.log(res);
      if(res == 'success'){
        (document.getElementById('uploadMsg') as HTMLInputElement).style.color = 'green';
        (document.getElementById('uploadMsg') as HTMLInputElement).style.visibility = 'visible';
